...
\vspace{1mm}
Note that the cluster you access may not use the standard 'port' number of \textsf{\footnotesize 22}. If your computer uses a different port number -- in the login window of your \textbf{sftp} client, simply change the number in the \textsf{\footnotesize Port} box.

Ir if you are using the command line, to specify a different port number you will need to type:
\vspace{-1mm}\begin{verbatim}
...
\noindent Having downloaded the code, you may ... or may not, need to configure some local environment settings so that all the libraries etc. that are needed to compile \textbf{cookie} can be found. To a very limited extent, \textbf{cookie} will try and identify your computer and make the required setting automatically. The changes, if any, that you need to make will depend on the platform where you will be running \textbf{cookie} -- the currently recognized platforms and required actions are as follows:
...
\noindent If your computer is configured to use python v.2 by default and/or does not have python v.3, then you will need to make a few minor changes (see HOW-TO). For a limited number of computer clusters, this can also be resolved by running \texttt{\small \$ ./installcookie.sh} from \texttt{\small genie-main}.
...
\vspace{-1mm}\begin{verbatim}
$ ./runcookie.sh cookie.C.p_worbe2.r LABS LAB.0.3 10
\end{verbatim}\vspace{-1mm}
...
\vspace{1mm}
\item Create a blank text file \footnote{\texttt{\$ touch file.txt} will achieve this at the linux command line.} and populate the contents with the parameter value assignments you need for your new experiment. But ... inevitably, it is difficult to remember all the names and even values you want to specify, meaning that you'll end up looking at existing \textit{user-config} files and copying and pasting lines (or even the entire contents) from the old file(s) into your new \textit{user-config} file. So then you may was well ...

\vspace{1mm}
\item {\uline Copy-rename, and edit an existing file}!
...
\noindent Remember that the new \textit{user-config} file needs to be saved in (or copied to) the \textsf{\footnotesize genie-userconfigs/LABS} sub-directory (or sub-directory of \textsf{\footnotesize LABS}) ... as long as you specify the correct path to the directory where you save the new file to when you run the experiment.\footnote{See previous Section and the use of the 2nd parameter passed to \texttt{runcookie.sh}}

%------------------------------------------------
\vspace{1mm}\noindent\rule{4cm}{0.5pt}\vspace{2mm}
%------------------------------------------------

\noindent For the purpose of trying this out, you can literally pick on any existing file in the \textsf{\footnotesize LABS } subdirectory (of \textsf{\footnotesize genie-userconfigs}). Note that you are not actually creating a \textit{different} (i.e., with altered parameter value(s)) experiment quite yet, just one with a new name.
...
\vspace{-1mm}
\small\begin{verbatim}
$ qsub -q QUEUE -j y -o cgenie_log -V -S /bin/bash 
  runcookie.sh cookie.C.p_worbe2.r LABS LAB.0.3 100
\end{verbatim}\normalsize
\vspace{-1mm}

Here, the queue name for this\ particular cluster is \textsf{\footnotesize QUEUE}. \uline{The cluster you are actually using will have a different queue name} (e.g. refer to any cluster-specific information that you might have).

\vspace{1mm}
It is important that you omit the ‘\texttt{./}’ bit before \texttt{runcookie.sh}.\footnote{If you are interested (I know that you are not): the options following \texttt{qsub} and before \texttt{runcookie.sh} do things like re-directing screen output and error messaging to a file and specify which linux ‘shell’ to assume.} 
