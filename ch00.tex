...
\chapterimage{ch0-cover.png} % Chapter heading image
...
\item The  low resolution (for a 3-D ocean circulation model) of the \textbf{cookie} model limits its applicability for very short time-scale problems.\ In configurations not incorporating the \textbf{PLASIM} atmospheric GCM component, there are no atmospheric dynamics or inter-annual variability in the coupled ocean-atmosphere system (and indeed, the ocean and atmosphere are not dynamically coupled without \textbf{PLASIM}).
\item \textbf{cookie} is best thought of as a ‘discovery and exploring’ tool for learning how the Earth system has functioned in the past rather than a detailed ‘simulation’ tool for the future (although it may be the only viable or most appropriate simulation tool for geological questions and events).
...
You will be using the current version of the cGENIE Earth system model, code-named ‘\textbf{cookie}’\footnote{If \textbf{Apple} can have ‘Leopard’, ‘Lion’ etc., I can have a baked goods version naming convention.}. For brevity, the \textbf{cGENIE.cookie} model will be referenced by just ‘\textbf{cookie}’.
...
For running \textbf{cookie}, your options are:

\begin{enumerate}[noitemsep]

\vspace{1mm}
\item \textbf{Remotely}
\\To do this, you will need an account on a linux-based server or cluster. You can use any platform (\textbf{linux}, \textbf{macOS}, \textbf{Windoz}, as well as \textbf{iOS} and \textbf{Android} (which is based on \textbf{linux})) to connect to the cluster. You will need the following software on your local machine:
\vspace{1mm}
\begin{enumerate}[noitemsep]
...
\item A sftp (secure file transfer) client for convenience (i.e. dragging and dropping files between local and remote computers, and opening files directly on the remote computer cluster). If you have installed ssh-client (\textbf{Windows}, above) then a sftp client is already included as part of this software. If using \textbf{PuTTY} (\textbf{Windows}) you might try downloading \textbf{WinSCP} (http://winscp.net/eng/index.php). For \textbf{macOS}, \textbf{sftp} software for viewing/navigating server file structure include: \textbf{FileZilla} (recommended), \textbf{Cyberduck}, \textbf{TextWrangler}. For linux, \textbf{FileZilla}.
\end{enumerate}
...
\begin{enumerate}[noitemsep]
\setlength{\itemindent}{.2in}
\item A FORTRAN (f90 and f77 combatable) compiler of some sort. This may come with the operating system as standard, possibly \textbf{gfortran}.
\item A \textbf{git} client. If not standard, this is relatively easy to add and install.
...
EITHER: Use the linux \textbf{vi} (/\textbf{vim}) application (or similar e.g. \textbf{emacs}) if you are familiar with it. I think that this pretty much sucks as a text editor and life is far too short and brutal if you don't like this sort of thing … OR ... use a suitable linux-friendly text editor (NOT \textbf{Micro\$oft} \textbf{Notepad}) in conjunction with the \textbf{Secure File Transfer Client}. For example: \textbf{SciTE} (http://www.scintilla.org/SciTE.html) is suitable, or better: \textbf{Notepad++}.
...
This (the \textbf{cookie} manual) lives on \href{https://github.com/genie-model/cookiedoc}{GitHub} -- The \textbf{latex} source for the documentation lives here, allowing you to compile the most up-to-date PDF document. 

In places, instructions may be given for specific programs and computer platforms and hence may differ slightly from the software reality in front of you. Use your judgment in translating such instructions. Many other alternative software choices exist for editing files or viewing results, as are other ways of configuring software and file editing/transferring methodologies. Do what suits you best – you can view such instructions where they occur, as  more representing an example methodology rather than a literal interpretation of the Constitution.

TLDR: The instructions may not be entirely bug-free -– use your judgment.

Lastly -- if you find yourself terminally bored typing in long long instructions lines, you may be tempted to simply copy-paste from the cookie manual PDF to the command line, This can work, but firstly be aware that trying to copy-paste multiple lines at once is doomed to failure -- copy-paste the first line and then following that add the second (or subsequent) line. Note that the inverted comma symbol in the PDF is not the same inverted comma symbol that linux is expecting ... You should also make liberal use of the up arrow key that bring back the previously entered command (keep pressing the key for progressively older commands). For instance, a mistake in a command line can be corrected by bringing back the offending line and using the left/right arrows to navigate through the characters and correct any mistake.
...
In running \textbf{cookie} locally -- log into your \textbf{linux}/\textbf{macOS} box (and skip on to the next section)! \footnote{If you fail at this step, you'll have to take up box-modeling instead.}

\noindent If however you are running \textbf{cookie} \uline{remotely} (e.g. via a user account on a computing cluster or server), then log into the remote server or cluster account using a 'suitable terminal program'\footnote{It very much depends on what software you are using. Provided are instructions for some \uline{examples}, but only examples, and your reality may be rather different.}:

\begin{itemize}
\vspace{1mm}
\item From \textbf{linux}/\textbf{macOS}, open the terminal/shell window and simply SSH in\footnote{If your current directory looks something like this: \texttt{[username@clustername \(\sim\)]\$}
...
\vspace{1mm}
\item From \textbf{Windoz} -– first start the \textbf{WinSCP} program (an \textbf{sftp} file transfer client). Under \textsf{\footnotesize Host Name}, enter the remote server or cluster name (e.g. \texttt{catname.ucr.edu})
\\Enter your computing cluster user-name on the line below this (‘\textsf{\footnotesize User Name}’) and then the \textsf{\footnotesize Password}. Click on \textsf{\footnotesize Login}.
...
\\(You can also log in directly from the shell/terminal window e.g. \textbf{PuTTY} first (rather than first opening an sftp connection first) but you will still have to open an \textbf{sftp} connection for file transfer.)
\end{itemize}

\vspace{1mm}
Note that the cluster you access may not use the standard 'port' number of \textsf{\footnotesize 22}. If your cluster uses a different port number -- in the login window of your \textbf{sftp} client, simply change the number in the \textsf{\footnotesize Port} box.

If you are using the command line -- to specify a different port number you will need to type:
\vspace{-1mm}\begin{verbatim}
...
\subsection{Downloading \& configuring the code}
...
\vspace{1mm} 
\noindent So ... basically do (1) unless you have a very good reason why not (e.g., you do not have \textbf{git} installed).
...
\noindent Having downloaded the code, you may ... or may not, need to configure some local environment settings so that all the libraries etc. that are needed to compile \textbf{cookie} can be found.

To a limited extent, \textbf{cookie} will try and identify your computer and make the required settings automatically. The changes, if any, that you need to make will depend on the platform where you will be running \textbf{cookie} -- the currently recognized platforms and required actions are as follows:

\begin{itemize}

\vspace{1mm}
\item \textbf{Ubuntu} -- the default assumed platform -- no changes should be necessary\footnote{If the \textbf{netCDF} libraries are installed in their default locations and a relatively recent version of \textbf{netCDF} is used.}.
...
\vspace{1mm}
\item \textbf{macOS} -- Please refer to the separate \textbf{macOS} instructions.

\vspace{1mm}
\item \textbf{Windows} -- Please refer to the separate \textbf{Windoz} instructions.

\vspace{1mm} 
\item \textbf{Otherwise} ...
\\... one or more edits may  be required to the file \textsf{\footnotesize user.mak}, which lives in \textsf{\footnotesize genie-main}.

\begin{enumerate}[noitemsep]
\vspace{2mm}
\item At the end of the \textsf{\footnotesize user.mak}, the \textbf{netCDF} path needs to be changed -- change the default setting for \texttt{\small NETCDF\_DIR=} to be the path to your \textbf{netCDF} libraries.
...
\noindent \textbf{GitHub} does not host files larger than 100 MB, and the 'lookup table' for calculating opal dissolution in sediments (see e.g., \textit{Ridgwell et al.} [2003]) is larger than this and hence is stored in a compressed format. If a silica cycle is to be employed in your \textbf{cookie} experiments -- from \texttt{\small genie-main}, type:

\vspace{-2mm}\small\begin{verbatim}
$ ./installcookie.sh
\end{verbatim}\normalsize\vspace{-2mm}

\noindent (This will unpack the opal lookup table to \textsf{\footnotesize genie-sedgem/data/input} as well as unpacking a copy of the calcium carbonate lookup table\footnote{For now, the \(CaCO_{3}\) lookup table also included in the git repository in its unpacked form.}.

\texttt{./installcookie.sh} will also attempt to identify if your computer is using python v.2 by default and/or does not have python v.3 install and will copy over a set of python2-friendly files to \texttt{\small genie-main}.
...
\noindent ‘Success’\footnote{If you see this -- you can then be certain that the model you have installed is producing identical (within tolerance) results to everyone else in the World who has ever installed and run \textbf{cookie}.} is indicated by:
\vspace{-1mm}\small\begin{verbatim}
**TEST OK**
\end{verbatim}\normalsize\vspace{-1mm}

Note that the model will pause for a l o o o o n g time at the line:
\vspace{-2mm}
\small\begin{verbatim}
...
Then and only then move on to tearing your hair out in frustration.
...
\noindent It requires that you must list at least 4 parameters after \texttt{./runcookie.sh}, separated by S P A C E S and on a single continuous line (even if it ‘wraps’ around across 2 lines of the screen).
...
\item[\textbf{\#2}] ... is the name of the sub-directory (if any) containing the user configuration (‘\textit{user-config}’) file (i.e., the file containing the specification of a particular experiment).
...
\begin{enumerate}[noitemsep]
\setlength{\itemindent}{.2in}
\item[\textbf{\#1}]: The \textit{base-config} is: \textsf{\footnotesize cookie.C.p\_worbe2.r}
\item[\textbf{\#2}]: The \textit{user-config} directory is: \textsf{\footnotesize  LABS}
\item[\textbf{\#3}]: The \textit{user-config} file (the experiment name) is: \textsf{\footnotesize LAB.0.3}.
\item[\textbf{\#4}]: Run the experiment for ten years: 10
\item[\textbf{\#5}]: (There is no \textit{restart} file (see later Section), and so no 5th parameter needs to be passed …)
\end{enumerate}

\vspace{1mm}
\noindent The full command for your first example experiment, which you are going to issue from the \textsf{\footnotesize cgenie.cookie/genie-main} directory, then looks like:

\vspace{-1mm}\begin{verbatim}
$ ./runcookie.sh cookie.C.p_worbe2.r EXAMPLES cookie.C.p_worbe2.r.SPIN 10
\end{verbatim}\vspace{-1mm}

\noindent(\uline{you can try it now}!)

\vspace{2mm}
\noindent NOTE: \textbf{The (single) S P A C E S are vital. }

\noindent ALSO -- \textbf{take care not to confuse an el (‘\texttt{l}’) with a one (‘\texttt{1}’)  ... (it is a ‘one’ here).}

%------------------------------------------------
\newpage
%------------------------------------------------

\noindent What should happen is: First, you will end up twiddling your thumbs a while, as all the components of \textbf{cookie} are compiled from the raw source code (\textbf{FORTRAN}). When it has finished doing this, the model will initialize and carry out some brief self-checking. Only then will it start actually ‘running’ and doing something ... in this particular example, the output should look something like this\footnote{You might want to increase the width of your terminal window in order to get the values reported for each year appearing on the same line and not wrapped-over to the next one.} :
 ...
 *******************************************************
 ...
 >   model year |                SAT(oC) |   AMOC(Sv)    ice(%)   SST(oC)  SSS(PSU)
  
 >          0.5 |                 -3.678 |     12.680     0.275     1.406    34.902
 >          1.5 |                 -2.568 |     13.073     2.151     3.440    34.902
 >          2.5 |                 -1.335 |     12.045     4.305     4.981    34.904
 >          3.5 |                 -0.365 |     10.681     5.499     6.177    34.906
 >          4.5 |                  0.515 |      9.403     6.090     7.133    34.908
 >          5.5 |                  1.303 |      8.380     6.443     7.919    34.910
 >          6.5 |                  1.996 |      7.581     6.640     8.584    34.912
 >          7.5 |                  2.609 |      7.403     6.706     9.157    34.913
 >          8.5 |                  3.145 |      7.388     6.815     9.658    34.914
 >>> SAVING BIOGEM TIME-SLICE AVERAGE CENTERED @ year  :        9.500
 >          9.5 |                  3.619 |      7.505     6.917    10.100    34.914
...
\item[] \texttt{---}
\item[] \texttt{<SAT>} -- global mean surface air temperature ('SAT') $^{\circ}$C
\item[] \texttt{---}
\item[] \texttt{AMO(Sv)} -- Atlantic meridional overturning circulation (AMOC) (Sv)\footnote{If you are not running an experiment with a modern-like Atlantic basin, no value is given for the AMOC.} 
...
\noindent In later experiments including the global carbon cycle, addition columns may appear, including:

\vspace{1mm}\begin{itemize}
\item[] \texttt{pCO2}  -- Atmospheric CO2.
\item[] \texttt{pH} -- Mean ocean surface pH.
\item[] \texttt{OHMEGA} -- Mean ocean surface calcium carbonate saturation state.
\item[] \texttt{O2} -- Global mean dissolved oxygen concentration in the ocean.
\item[] \texttt{fPOC} -- Global total annual export production.
\end{itemize}\vspace{1mm}
...
\noindent The choice of what information is displayed on screen as the model is running is rather arbitrary, but the chosen metrics do tend to summarize some of the main properties of the climate system (and carbon cycle).
...
So, in the model experiment configuration you have been given -- there is 'time-series' saving every year, but only once at the end (or it could be every 10 years) for 'time-slice'. You will see what this all means shortly.
...
\vspace{2mm}
\caption{Schematic of the sequence-of-events involved in configuring and running an experiment.}
...
The key to creating new experiments is to remember that \textbf{the name of the \textit{user-config} file, that contains the parameter settings that define that specific experiment, becomes the name of the experiment} and hence name of the model results sub-directory in \textsf{\footnotesize cgenie\_output}. Changing the name of the \textit{user-config} file hence leads to a new experiment name and a new model results sub-directory. (Conversely, not changing the name of the  \textit{user-config} file and re-running the model results in the results of any previous experiment run using that \textit{user-config} file being over-written.)
...
\\But ... inevitably, it is difficult to remember all the names and even values you want to specify, meaning that you'll end up looking at existing \textit{user-config} files and copying and pasting lines (or even the entire contents) from the old file(s) into your new \textit{user-config} file. So then you may was well ...

\vspace{1mm}
\item \uline{Copy-rename, and then edit an existing file}!
...
\vspace{1mm}
\item Via your \textbf{sftp} client (program):
\begin{enumerate}[noitemsep]
\item Drag the existing \textit{user-config} file to your local computer. On your local computer, rename the file as per how you would 'normally' rename any file. 
\item Edit the parameter values and then save the file.
\item Drag the file back to the cluster (from your local computer) using the \textbf{sftp} client.
\end{enumerate}
...
\vspace{1mm}
\item Alternatively, if you are comfortable working at the linux command line; working from the same directory (e.g. \textsf{\footnotesize LABS}) that the file you want to copy lives in, you can copy a file (to a new filename) by:
...
\noindent Remember that the new \textit{user-config} file needs to be saved in (or copied to) \textsf{\footnotesize genie-userconfigs} or better, a sub-directory of this, e.g., \textsf{\footnotesize LABS} (or a sub-directory of the sub-directory ...) and then specify the name of the sub-directory when you run the experiment (or if no sub-directory, pass '\textsf{\footnotesize /}').\footnote{See previous Section and the use of the 2nd parameter passed to \texttt{runcookie.sh}}

%------------------------------------------------
\vspace{1mm}\noindent\rule{4cm}{0.5pt}\vspace{2mm}
%------------------------------------------------

\noindent For the purpose of trying this out, you can literally pick on any existing file in the \textsf{\footnotesize LABS} or \textsf{\footnotesize EXAMPLES} sub-directory (of \textsf{\footnotesize genie-userconfigs}).
...
Experiment results are saved in a single sub-directory of the experiment results directory (along with a sub-directory for \textit{restart} files, and one for copies of your parameter choices). The directories containing the results of all your different experiment live in:
...
LAB.0.3
\end{verbatim}\normalsize\vspace{-1mm}
(this being the results directory name for an experiment called \textsf{\footnotesize LAB.0.3}) and the actual results results live in a sub-directory of this -- \textsf{\footnotesize results}, hence:

\vspace{-1mm}\small\begin{verbatim}
~/cgenie_output/LAB.0.3/results
\end{verbatim}\normalsize\vspace{-1mm}

\noindent NOTE: If in an \textbf{sftp} client window you find that you cannot 'see' the \textsf{\footnotesize cgenie\_output} directory ... or cannot find any of the results sub-directories you are expecting, \uline{you will need to refresh the directory listing} (e.g. for \textbf{WinSCP}, there is a double green-arrow \textsf{\footnotesize Refresh} icon button near the top right of the window that you can click on). \textbf{sftp} client programs generally do not automatically refresh directory listing on the remote computer.
...
One of the most informative data sets that can be saved is that of the spatial distribution of properties (such as tracers or physical ocean attributes). However, saving full spatial distributions (e.g., a 36\(\times\)36\(\times\)8 array) for any or all of the model variables each and every time-step may not be not practical in terms of data storage. Instead, \textbf{cookie} is programmed to save the full spatial distribution of tracer properties only at one or more predefined time points (in units of years). These are termed \textit{time-slices}. At the specified time points -- a set of spatially-explicit data fields are saved for all the key tracer, flux, and physical characteristics of the system. However, rather than taking an instantaneous snapshot, the time-slice is constructed as an average over a specified integration interval (the default is set to 1.0 years, i.e. an annual average). \textbf{cookie}  assumes that the specified time point represents the mid-point of the (annual) average with the results that output years end up being reported.
...
\end{verbatim}\normalsize\vspace{-1mm}
and which would get save points at:
...
\noindent Otherwise, you can specify the name of a file containing times to save at, or simply only save just once at the very end of the experiment (see later).
...
\begin{itemize}[noitemsep]
\setlength{\itemindent}{.2in}
\item \textsf{\footnotesize fields\_ocean\_2d.nc} – 2-D fields of ocean and atmosphere properties, as \textbf{NetCDF}.
\item \textsf{\footnotesize fields\_ocean\_3d.nc} – 3-D fields of ocean properties, as \textbf{NetCDF}.
\item \textsf{\footnotesize INFO\_ocean\_summary\_AT\_*\_yrs.txt} – these contain (global diagnostics) summary information and are saved at the same frequency as the \textit{time-slices} (in ASCII / plain text format).
\item \textsf{\footnotesize timeseries\_*.txt} – these are the \textit{time-series} files (in ASCII / plain text format).
\end{itemize}
\vspace{1mm}
...
Having the e.g., 2D and 3D \textbf{netCDF} files always called the same name (\textsf{\footnotesize fields\_ocean\_2d.nc}, \textsf{\footnotesize fields\_ocean\_3d.nc}) in each and every experiment, has the potential to get confusing if you have multiple experiments open simultaneously in a viewer such as \textbf{Panoply}.
...
A descriptive summary of all the time-series (\textsf{\footnotesize biogem\_series\_*.txt}) data files is given in the \textbf{cookie} User Manual if you are really that bored. The files of most immediate use/relevance are:

\vspace{1mm}
\begin{itemize}[noitemsep]
\setlength{\itemindent}{.2in}
\item \textsf{\footnotesize timeseries\_climate\_humidity.txt} - mean atmospheric (surface) humidity
\item \textsf{\footnotesize timeseries\_climate\_SAT.txt}      - mean atmospheric (surface) air temperature
\item \textsf{\footnotesize timeseries\_climate\_SLT.txt}      - mean atmospheric (surface) air temperature
\item \textsf{\footnotesize timeseries\_climate\_SSS.txt}      - mean ocean surface and whole ocean salinity
\item \textsf{\footnotesize timeseries\_climate\_SST.txt}      - mean ocean surface and whole ocean temperature
\item \textsf{\footnotesize timeseries\_climate\_opsi.txt}     - overturning stream-function (e.g. AMOC) strength
\item \textsf{\footnotesize timeseries\_climate\_seaice.txt}   - mean ocean sea-ice cover and thickness
\end{itemize}

\noindent plus 2 outputs for solar radiation / short-wave flux, and depending on the configuration, idealized ocean ventilation age.
...
Instead – change to the experiment results directory and then to the \textsf{\footnotesize results} sub-directory in the \textbf{sftp}, and try double-clicking (if you have set up the \textbf{WinSCP} preferences correctly) or right-mouse-button-clicking (the then \textsf{\footnotesize Edit with}) on one of the .txt files (listed above). For most of these basic climate outputs, you should see just a column for time, and one for the variable value.\footnote{For some other properties such as ocean tracers, you might get up to 5 columns:
\texttt{\% time (yr) / temperature (C) / \_surT (ice-free) (C) / \_benT (C) / \_surT (C)}
for: model time (years) (actually, the mid-point in time of an annual average), annual mean ocean temperature (averaging over the entire ocean volume), annual mean ocean surface temperature (excluding ice-cover areas), annual mean benthic (sea-floor) temperature, annual mean ocean surface temperature (now including ice-cover areas).} 
...
Running jobs interactively is all very well, but there are three important limitations:
\begin{enumerate}[noitemsep]
\vspace{1mm}
...
\vspace{1mm}
\item You can only run one experiment at a time … unless you want to have thousands of separate terminals open …? I thought not …
\vspace{1mm}
\item Any cluster that you have access to with be designed for submitting jobs, and not trying to run everything interactively.
\end{enumerate}
...
For example -- to run the same experiment as before (\textsf{\footnotesize LAB.0.3}) for maybe 100 years (or even longer if you wish – I am just pulling factors of 10 out of thin air here) but now submit the experiment as a job to the cluster queue, type (again: SINGLE, CONTINUOUS LINE):
\vspace{-1mm}
\small\begin{verbatim}
$ ./subcookie.sh cookie.C.p_worbe2.r LABS LAB.0.3 100
\end{verbatim}\normalsize
\vspace{-1mm}

Literally, you are just changing \textsf{\footnotesize ./runcookie.sh} to \textsf{\footnotesize ./subcookie.sh} (which is much simpler than was the case for the \textbf{muffin} release of \textbf{cGENIE}). A listing of your running experiments on the cluster will then be automatically displayed after a whort pause.

%------------------------------------------------
\vspace{1mm}\noindent\rule{4cm}{0.1mm}\vspace{2mm}
%------------------------------------------------

\noindent You can request the status of the cluster queue and how you experiment job is getting on (e.g., “Is it finished yet?”) at any time by typing:
\vspace{-1mm}\small\begin{verbatim}
$ qstat -f
\end{verbatim}\normalsize\vspace{-1mm}
...
For assessing how long you might run submitted jobs for -- a rough rule of thumb is that 8-level ocean \textbf{cookie} @ a horizontal grid resolution of 36x36 will simulate about 1000 years per CPU hour. The 16-level version (which you will use later), runs at about 300-400 years per CPU hour.
...
\noindent To experiment with using a \textit{re-start}, you will first need to download a file that has been created (a pre-run 10,000 year spin-up).

\vspace{1mm}
To fetch this: change to the \textsf{\footnotesize cgenie\_output} directory (perhaps by going ‘home’ first (\texttt{cd} \textsf{\footnotesize <Enter>}), and then changing to \textsf{\footnotesize cgenie\_output} – refer to linux commands HOW-TO and Figure 1.1). Then type (all one line!):

\vspace{-2mm}\small
\begin{verbatim}
$ wget --no-check-certificate http://www.seao2.info/cgenie_output/
   cookie.C.p_worbe2.r.SPIN.tar.gz
\end{verbatim}\normalsize
\vspace{-2mm}

This downloads an archived/compressed copy of the \textit{re-start} from a location on the interweb. Extract the contents of this archive by typing:

\vspace{-2mm}\small\begin{verbatim}
$ tar xfzv cookie.C.p_worbe2.r.SPIN.tar.gz
\end{verbatim}\normalsize\vspace{-2mm}
...
\textsf{\footnotesize cookie.C.p\_worbe2.r.SPIN} -- which you have just unpacked to the \textsf{\footnotesize cgenie\_output} results output directory.
...
\noindent taking the file \textsf{\footnotesize LAB\_0.3} (which is provided) as a template (no parameter changes need to be made yet). As described earlier -- copy this file and give it a new name -- here, name it: \textsf{\footnotesize LAB.0.3.NEW}

\noindent You can then specify the use of the \textit{restart} in your new \textsf{\footnotesize LAB\_0.3.NEW} experiment:

\vspace{-2mm}
\small\begin{verbatim}
$ ./runcookie.sh cookie.C.p_worbe2.r LABS LAB.0.3.NEW 100 cookie.C.p_worbe2.r.SPIN
\end{verbatim}\normalsize
\vspace{-2mm}
...
\section{linux and git 101}

%------------------------------------------------

\subsection*{Some brief notes on linux}
...
\vspace{2mm}\begin{itemize}[noitemsep]
\setlength{\itemindent}{.2in}
\item :q == quit
...
\subsection*{Some brief notes on git}
...
\noindent which will 'download [new and modified] remote content but not update your local repo's working state'. If you now type \texttt{git status -uno}, \textbf{git} can tell you if there is newer content (e.g. '\texttt{Your branch is behind 'origin/main}'\footnote{\texttt{origin} is the origin of the repo -- GitHub, and \texttt{master} is the name of the branch, in this case, the default branch name.}). To merge in the fetched content:
