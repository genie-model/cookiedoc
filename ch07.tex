...
\vspace{2mm}
We will use some non ecology-enabled, biogeochemical cycles \textit{restarts} from the previous Chapter (specifically: \textsf{\footnotesize cookie.CB.p\_worjh2.BASES.caoetal.SPIN} and \textsf{\footnotesize  cookie.CB.p\_worjh2.BASESFe.FeMIP.SPIN}) which comprise a stable ocean circulation and a cycle of nutrients already modified by the biological pump (although we are not going to explore changing the mechanistic control on biological productivity).
...
$ wget --no-check-certificate 
     http://www.seao2.info//cgenie/data/GEnIE_observations.nc
...
We will start with the simplest possible configuration of \textbf{ECOGEM}, with just a single (small) phytoplankton class\footnote{i.e. as if the ocean was populated with a small species of phytoplankton (photo-autotroph) and nothing else.}. You can run an experiment (e.g. for 10 years) based on this configuration:
\vspace{-1mm}\small\begin{verbatim}
$ ./runcookie.sh cookie.CBE.p_worjh2.BASES LABS LAB.7.1.EXAMPLE 10 
   cookie.CB.p_worjh2.BASES.caoetal.SPIN
\end{verbatim}\normalsize\vspace{-1mm}
(Here, although you are using a new \textit{base-config} -- \textsf{\footnotesize cookie.CBE.p\_worjh2.BASES} -- it is identical to the 16-level phosphorous-only biogeochemical configuration used in the previous Chapter except now with the ecosystem model enabled (the '\textsf{\footnotesize E}' in '\textsf{\footnotesize cookie.CBE.p\_worjh2.BASES}').)

The model will run as before ... except much slower ... :( ... as \textbf{cookie} has to calculate plankton growth and ecological interactions in addition to everything else it was doing previously.
...
\vspace{1mm}
\noindent \textbf{ECOGEM} saves its own 2D (but not 3D) netCDF \textit{time-slice} output (also in the \textsf{\footnotesize results} sub-directory of your experiment output folder).

\begin{table}[htp!]
\begin{tabular}{p{0.25\linewidth} p{0.20\linewidth} p{0.25\linewidth} p{0.20\linewidth}}
\toprule
\textbf{variable} & \textbf{variable} & \textbf{Description} & \textbf{Application}\\
 & (long name) & &\\
\midrule

\textsf{\footnotesize eco2D\_Plankton\_C\_x} & \textsf{\footnotesize C Biomass - Popn. \#x} & 
\small{Biomass ($mmol C m^{-3}$) of phytoplankton or zooplankton [OF GIVEN SIZE CLASS].} & \small{Relative dominance of different size classes.\footnote{Relative cellular numbers can be derived using cell volume.} Comparison to observations.}\\
\textsf{\footnotesize eco2D\_Plankton\_P\_x} & \textsf{\footnotesize P Biomass - Popn. \#x} & 
\small{Biomass ($mmol P m^{-3}$) of phytoplankton or zooplankton [OF GIVEN SIZE CLASS].} & \small{(as above)}\\
\textsf{\footnotesize eco2D\_Plankton\_Chl\_x} & \textsf{\footnotesize Chl Biomass - Popn. \#x} & 
\small{Biomass ($mg Chl m^{-3}$) of phytoplankton [OF GIVEN SIZE CLASS].} & \small{Comparison to observations.}\\

\midrule

\textsf{\footnotesize eco2D\_Plankton\_C\_x} & \textsf{\footnotesize C Biomass - Total} & 
\small{Total ecosystem biomass ($mmol C m^{-3}$)} & \small{--}\\
\textsf{\footnotesize eco2D\_Plankton\_P\_x} & \textsf{\footnotesize P Biomass - Total} & 
\small{Total ecosystem biomass ($mmol P m^{-3}$)} & \small{--}\\
\textsf{\footnotesize eco2D\_Plankton\_Chl\_x} & \textsf{\footnotesize Chl Biomass - Total} & 
\small{Total ecosystem biomass ($mg Chl m^{-3}$)} & \small{Comparison to observations, e.g., satellite retrieval.}\\

\midrule

\textsf{\footnotesize eco2D\_Size\_*} & \textsf{\footnotesize --} & 
\small{Various size-related metric.} & \small{Analysis of ecosystem changes.}\\

\midrule

\textsf{\footnotesize eco2D\_Uptake\_Fluxes\_x} & \textsf{\footnotesize Uptake Fluxes x} & 
\small{Rate of nutrient (can carbon) update.} & \small{Primary productivity.}\\

\midrule

\textsf{\footnotesize eco2D\_xGamma\_*} & \textsf{\footnotesize Limitation [BY NUTRIENT] [OF GIVEN SIZE CLASS]} & 
\small{Various size-related metric.} & \small{Analysis of ecosystem changes.}\\

\midrule

\textsf{\footnotesize eco2D\_Diversity\_*} & \textsf{\footnotesize --} & 
\small{Nutrient limitation of growth.} & \small{Analysis of ecosystem controls.}\\

\midrule

\textsf{\footnotesize eco2D\_Nutrients\_*} & \textsf{\footnotesize Nutrients *} & 
\small{Ocean surface nutrient concentrations including that of DIC, in units of \(mmol m^{-3}\).} & \small{(Duplicate of \textbf{BIOGEM} fields ... but different units.)}\\

\bottomrule
\end{tabular}
\caption{Summary of the main 2D time-slice output for ecology.}
\end{table}
...
\vspace{1mm}
\item Download a compilation of key biogeochemical variables -- \\\textsf{\footnotesize http://www.seao2.info/cgenie/data/GEnIE\_observations.nc} \href{http://www.seao2.info/cgenie/data/GEnIE_observations.nc}.
\vspace{1mm}
\item Compare model output to key biogeochemical variables derived from ocean measurements.\footnote{Remember that you can create difference maps.}  The variables in the \textsf{\footnotesize GEnIE\_observations.nc} \textbf{netCDf} file include:
\vspace{1mm}
\begin{itemize}[noitemsep]
\item Phosphate -- how well does \textbf{cookie} simulate surface nutrient availability, and to what degree does ecosystem complexity help explain observations?
\item Observed Chlorophyll is remote-sensed, and can be contrasted with the chlorophyll biomass simulated by \textbf{ECOGEM}.
\end{itemize}
\vspace{1mm}
\item Does the model perform well with respect to reproducing these variables? If not, \uline{why not}?
\end{enumerate}
\vspace{2mm}
...
\noindent \textsf{\footnotesize NPD.eco} note that only the lines in between the \texttt{\small -START-OF-DATA-} and \texttt{\small -END-OF-DATA-} tags are read by the computer. The rest is there solely for your guidance.
...
\vspace{2mm}
\item You can increase the ecological complexity of the model by adding another plankton population. Save the \textit{ecosystem configuration} file under a new and highly intuitive name (such as \textsf{\footnotesize NP\underline{Z}D.eco}), and add another line specifying a 100 micron zooplankton. e.g.:
\small\begin{verbatim}
...
We can perhaps get a better handle on this question by looking at the ratio of phytoplankton-to-zooplankton biomass. Use \textbf{Panoply} to combine data arrays.

\vspace{1mm}
\begin{enumerate}[noitemsep]
\vspace{1mm}
\item Open \textsf{\footnotesize C~Biomass - Popn.~001 (10.00~micron phytoplankton)}. Next, select \textsf{\footnotesize C~Biomass - Popn.~002 (100.00~micron zooplankton)}, and click the \textsf{\footnotesize Combine Plot} icon at the top of the \textbf{Panoply} window.
\vspace{1mm}
\item A box will open up asking you \textsf{\footnotesize In which existing plot should I combine the variable}. Click \textsf{\footnotesize Combine}.
\vspace{1mm}
\item The new map shows total zooplankton carbon biomass minus the total phytoplankton carbon biomass (see the label on the color scale). We want to look at the Z:P biomass ratio, so from \textsf{\footnotesize Array(s)}, select \textsf{\footnotesize Array 2 / Array 1}.\footnote{Make sure that you are looking at the right year for both.} You may find it helpful to look at the data on a log scale, with a scale range of \textsf{\footnotesize 0.1} to \textsf{\footnotesize 10}. You might also like to change the \textsf{\footnotesize Color Table:} option to \textsf{\footnotesize GMT\_polar.cpt}.
\end{enumerate}
...
\vspace{1mm}
\item Save the new \textit{user-config} file under a different name (e.g., \textsf{\footnotesize LAB.7.2.3P3Z}) and then run the new model at the command line (e.g. for 10 years).
\end{enumerate}
\vspace{2mm}
...
View the spatial distributions of some of these metrics, but bear in mind that they summarize the diversity of a phytoplankton community that includes just three species. They are probably not that revealing, so we will come back to them later. Instead, have a look some of the other metrics describing the model ecosystem.
...
\vspace{1mm}
\item Create a new \textit{user-config} (e.g., \textsf{\footnotesize LAB.7.4.8P8Z}) and point to the new \textit{ecosystem configuration} file, e.g., (e.g. \textsf{\footnotesize 8P8Z.eco}).
\vspace{1mm}
...
\end{enumerate}
\vspace{1mm}
Make sure that in each case that you are looking at the \uline{last} year of model output. You may also find it useful to adjust the color scale, or to change to a logarithmic colour scale.
...
\item Sub-polar latitudes between 40$^\circ$ and 50$^\circ$ (either N or S) are much more productive, and support very high annual mean biomass. These communities also have the highest mean sizes of any region.
...
\vspace{1mm}
The fundamental niche is a fairly abstract concept, and not something that can be measured in the real world. In model world, however, we can get a useful estimate of the fundamental niche by making a few simple changes to the model.
...
\begin{enumerate}[noitemsep]
\vspace{1mm}
\item All you need to do at this stage is to re-enable nutrient and light competition. To do this, simply delete `\texttt{\small eg\_fundamental~=~.TRUE.}' from the \textit{user-config} file (or comment out the line to disable it), and save under a new name. Leave the \textit{ecosystem configuration} file as it is.
...
What might happen to primary production and the biological pump as (future) climate continues to change? What might happen to the distribution of species (here: size classes) in the ocean? Does it 'matter' e.g. for marine bio geochemical cycling and feedback on atmospheric \(pCO_{2}\)? (And how different is the Earth system response when using a complex ecosystem model rather than the highly simplified representation of biological export you used previously?)

\vspace{1mm}
There are all good questions for a model!

\vspace{1mm}
The best place to start is with the full complexity ecosystem structure with 8 size classes for both phytoplankton and zooplankton (as described in \textit{Ward et al.} [2018]). However, you do not have a \textit{restart} for this -- so far, you were adding an ecosystem to a distribution of nutrients and carbon cycling derived from a much simpler description of biological export production.

\begin{enumerate}[noitemsep]

\vspace{2mm}
\item First, create a \textit{restart} that includes a complete ecosystem. You could run this for 10,000 years (it would take 3 days!). But you might get away with e.g., 1,000 years following on from the non-ecosystem \textit{restart}. Copy your previous full-complexity ecosystem \textit{user-config} (\textsf{\footnotesize LAB.7.4.8P8Z}) to e.g., \textsf{\footnotesize LAB.7.6.8P8Z} and run it:

\vspace{-1mm}\small\begin{verbatim}
$ ./runcookie.sh cookie.CBE.p_worjh2.BASES LABS LAB.7.6.8P8Z 1000
   cookie.CB.p_worjh2.BASES.caoetal.SPIN
\end{verbatim}\normalsize\vspace{-1mm}

Even 1,000 years might not be long enough ... check the \textit{time-series} of POC export and surface nutrient concentrations and judge whether steady-state has been reached or not.

\vspace{2mm}
\item You then need to run a historical transient experiment (e.g., as per in Chapter 5). To adapt your \textit{spin-up} experiment as a historical transient, add the following lines to the \textit{user-config}.

\vspace{1mm}
\begin{itemize}[noitemsep]

\vspace{1mm}
\item Under \texttt{\small \# *** DATA SAVING ***}, make sure you change the existing line to the following, or add these at the \uline{end} of the parameter block:
\vspace{-1mm}\small\begin{verbatim}
bg_par_data_save_sig_timeinterval=0.0
...
\end{verbatim}\normalsize\vspace{-1mm}
These simply ensure you are saving output at appropriate years/intervals.

\vspace{1mm}
\item Under \texttt{\small \# *** FORCINGS ***} and in place of any existing forcing lines:
\vspace{-1mm}\small\begin{verbatim}
# historical forcing of pCO2
bg_par_forcing_name='pyyyyz.RpCO2_Rp13CO2.historical2010'
\end{verbatim}\normalsize\vspace{-1mm}
This prescribes increasing atmospheric CO2 following historical observations.

\vspace{1mm}
\item And then under \texttt{\small \# *** MISC ***}:
\vspace{-1mm}\small\begin{verbatim}
# change start year
bg_par_misc_t_start=1765.0
\end{verbatim}\normalsize\vspace{-1mm}
Starts the experiment at year 1765.

\end{itemize}

Run this experiments, starting form the end of your \textit{spin-up}, for 245 years, which will take it up until year 2010.

\vspace{2mm}
\item To further adapt your \textit{user-config} to run future CO2 scenarios -- see Section 5.2.3.
...
In general, any of the different \textit{user-configs} you have created and experimented with, you be applied to a paleo \textit{base-config}. Primarily, only the atmospheric the atmospheric CO2 forcing values needs changing in order to simulate e.g., a warmed past climate state.
