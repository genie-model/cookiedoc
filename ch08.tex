...
There is a whole new set of additional outputs from this configuration of \textbf{cookie}, particularly sediment-specific output, which is saved along with all the other results files in \textsf{\footnotesize results} sub-directory of the main experiments results directory. However, sediment model output differs from before in that the saving of spatial, \textbf{netCDF} output, only occurs \uline{at the very end} of a model experiment. So if you kill a run before the very end (or it crashes), \uline{you will get no} (or little) sediment output ...

%------------------------------------------------
\vspace{1mm}\noindent\rule{4cm}{0.1mm}\vspace{2mm}
%------------------------------------------------

\noindent To learn about the basic geological carbon cycle output saved by cookie, run a short experiment starting from the \textit{restart} you downloaded -- \textsf{\footnotesize cookie.CBSR.p\_worbe2.BASES.RidgwellHargreaves.SPIN2} -- as follows:
...
\subsubsection*{Sediments of the modern Earth}

Plot the surface ('core-top') sediment composition ($wt\%CaCO_{3}$). Compare with the plotted observed distribution of core-top sediment composition in \textit{Ridgwell and Hargreaves} [2007].

\textbf{Q.} Does the model get the broad patterns right (is it more right than wrong, or more wrong than right)? Do you think the model-data misfits might be important? How so?
...
\subsection*{Simulating sediment records ('\textit{sedcores}')}
...
If you request that sediment properties are saved as a function of depth at specific locations as sediment cores ('\textit{sedcores}'), there will be an additional sediment model \textbf{netCDF} output: \textsf{\footnotesize sedcore.nc}. Saved include:
