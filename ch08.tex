...
You will need to download a new \textit{re-start} file.
\\To fetch this -- change to the \textsf{\footnotesize cgenie\_output directory}, and type:
\vspace{-1mm}\small\begin{verbatim}
$ wget --no-check-certificate http://www.seao2.info/cgenie_output/
   cookie.CBSR.p_worbe2.BASES.RidgwellHargreaves.SPIN2.tar.gz
...
This configuration of \textbf{cookie} generates additional \textit{time-series}  output in the form of \textit{time-series} of sediment properties: \\\textsf{\footnotesize timeseries\_sed\_seafloor\_*.txt}. For example, the \textit{time-series} file: \textsf{\footnotesize timeseries\_sed\_seafloor\_CaCO3.txt} contains information about how the mean \(CaCO_{3}\) content of surface sediments (uppermost sediment layer) evolves with time. There are also \textit{time-series} file output that record the evolution of burial (and dissolution) fluxes as well as weathering -- all of which will be important for fully understanding system behavior. (But note that in the default output, only key sources and sinks are saved -- \textsf{\footnotesize timeseries\_geo\_*.txt}.)

As per with the ocean (-atmosphere) data saving, a summary file of sediment fluxes and composition is also saved -- \textsf{\footnotesize INFO\_sediment\_summary\_AT\_END.txt} (but only at the experiment end, rather than associated with each \textit{time-slice}) as well as a parameter value summary for use in configuring weathering (discussed later) -- \textsf{\footnotesize INFO\_weathering\_parameters\_AT\_END.txt} 

%------------------------------------------------
\vspace{1mm}\noindent\rule{4cm}{0.1mm}\vspace{2mm}
%------------------------------------------------

\noindent 2D (e.g. surface sediment properties, fluxes, etc.) output can also be found in the \textsf{\footnotesize results} sub-directory of your experiment directory in a \textbf{netCDF} file called \textsf{\footnotesize fields\_sediments\_2d.nc}\footnote{Note that there is some duplication of results saving, because a series of \textit{time-slices} of sediment composition are also saved in the 2D \textbf{netCDF} file \textsf{\footnotesize fields\_ocean\_2d.nc}.}. For example, the 2D distribution of \(wt\% CaCO_{3}\) –- which is the weight fraction of calcium carbonate (\(CaCO_{3}\)) in the surface sediments of the deep ocean (i.e., how much plankton carbonate shell material is there compared to other stuff in the mud at the bottom of the ocean) -- is saved here\footnote{In a variable called: \textsf{\footnotesize sed\_CaCO3}}. 

\vspace{1mm}
\noindent NOTE: The saving of spatial, \textbf{netCDF} output, only occurs \uline{at the very end} of a model experiment. So if you kill a run before the very end (or it crashes), you will get no (or little) sediment output ...

%------------------------------------------------
\vspace{1mm}\noindent\rule{4cm}{0.1mm}\vspace{2mm}
%------------------------------------------------

\subsection{Simulating the marine geological record ('\textit{sedcores}')}

\textbf{cookie} can generates sediment records -- ‘cores’ (e.g. see: \textit{Ridgwell} [2007]) -- and hence simulates what one might expect to see in a marine sediment core recovered from the ocean floor. The locations of these cores are either stored in a file listing their locations\footnote{Parameter: \textsf{sg\_par\_sedcore\_save\_list\_name} contains a filename.} (see later), or in a file containing an ASCII ‘map’ of the ocean. The ASCII ‘map’ filename is given by the parameter: \texttt{\small sg\_par\_sedcore\_save\_mask\_name} Simply be changing (using a text editor) a '\texttt{\small 0.0}' to a '\texttt{\small 1.0}', you can get the model to generate and save a sediment ‘core’ at that particular (\textit{i,j}) location.

For example, in the folder: \textsf{\footnotesize genie-paleo/p\_worjh2}, the file \textsf{\footnotesize worjh2\_save\_mask.36x36} contains 5 '\texttt{\small 1}s' clustered in the Eastern Mediterranean basin. Note that \textbf{cookie} does not distinguish between a '\texttt{\small 00}' and a '\texttt{\small 0}' -- this is simply to help you orientate yourself and identify land ('\texttt{\small 00}') vs. ocean with no sedcore defined ('\texttt{\small 0}'). 

%------------------------------------------------
\vspace{1mm}\noindent\rule{4cm}{0.1mm}\vspace{2mm}
%------------------------------------------------

\noindent Additional output is available in the form of \textit{time-series} of sediment properties: \\\textsf{\footnotesize timeseries\_sed\_seafloor\_*.txt}. For example, the \textit{time-series} file: \textsf{\footnotesize timeseries\_sed\_seafloor\_CaCO3.txt} contains information about how the mean \(CaCO_{3}\) content of surface sediments (uppermost sediment layer) evolves with time. There are also \textit{time-series} file output that record the evolution of burial (and dissolution) fluxes as well as weathering -- all of which will be important for fully understanding system behavior. (But note that in the default output, only key sources and sinks are saved -- \textsf{\footnotesize timeseries\_geo\_*.txt}.)
...
\noindent Sediment \textit{cores} are saved in the \textsf{\footnotesize results} sub-directory of your experiment directory in a \textbf{netCDF} file called \textsf{\footnotesize fields\_sediments\_1d.nc}. Individual sediment \textit{cores} are assigned an ID number starting at $1$, with the spatial pattern of IDs saved in \textsf{\footnotesize fields\_sediments\_2d.nc} as variable \textsf{\footnotesize grid\_ID\_sedcore}. In \textsf{\footnotesize fields\_sediments\_1d.nc}, these IDs form the \textit{x}-axis for all the saved variable fields, with accumulated sediment depth as the \textit{y}-axis. NOTE: When opening the \textsf{\footnotesize fields\_sediments\_1d.nc} \textbf{netCDF} file you may need to switch the Panoply file contents display settings at the bottom from \textsf{\footnotesize Show: Georeferenced variables} to \textsf{\footnotesize Show: Plottable variables} (or \textsf{\footnotesize Show: All variables}) because the \textit{x}-axis is not a spatial dimension (such as latitude).

Sediment \textit{cores} reflect not only the material which has accumulated (or not accumulated, or accumulated and subsequently dissolved …) during the course of your experiment, but also the material that has accumulated during any \textit{re-start}. PLUS, whatever material the sediment \textit{core} was initialized with to start with. For example, the large interval of initially 100\% detrital material at the base of the sediment core simply reflects the initialization of the sediment array in the model. Also note the ash ‘peak’ near the bottom of the stack (filled) sediment layers – this is a tracer to ‘tag’ the start of the model spin-up. If you look at the spin-up results (not your recent perturbation experiment) – the ash peak lies in a sediment layer with age equal to the total length of of the spin-up carried out.\footnote{\textbf{Q.} Why is there any ash deeper than the age corresponding to the start of the spin-up? How can it get there (i.e. what processes could move solid material deeper within the sediment column)?} Variables include:
...
\item \textsf{\footnotesize age\_CaCO3 } -- the mean age of \(CaCO_{3}\) particles in a sediment layer (\uline{if} there is any \(CaCO_{3}\) preserved), and then some alternative ways of assigning a sediment chronology ...
...
\end{itemize}
\vspace{1mm}
and depending on the (isotope) tracers selected in the specific \textit{base-config}:
\vspace{1mm}
\begin{itemize}[noitemsep]
...
\item \textsf{\footnotesize sed\_POC } and \textsf{\footnotesize sed\_POC\_13C } -- mean organic matter content of each sediment layer and its \(\delta^{13} C\). (In common configurations no organic matter is preserved and hence wt\% POC will be zero.)
\end{itemize}
...
\noindent Published examples\footnote{Think about how you might make use of the \textit{sedcore} output in light of these paper} of simulating the marine geological (as sediment cores) include (in order of publication date):
\vspace{1mm}
\begin{itemize}[noitemsep]
...
To learn about the basic geological carbon cycle output saved by cookie, run a short experiment starting from the \textit{restart} you downloaded -- \textsf{\footnotesize cookie.CBSR.p\_worbe2.BASES.RidgwellHargreaves.SPIN2} -- as follows:

\vspace{-1mm}\small\begin{verbatim}
./runcookie.sh cookie.CBSR.p_worbe2.BASES LABS LAB.8.1 100 
...
\noindent Plot the surface ('core-top') sediment composition ($wt\%CaCO_{3}$). Compare with the plotted observed distribution of core-top sediment composition in \textit{Ridgwell and Hargreaves} [2007].

\vspace{1mm}
\noindent \textbf{Q.} Does the model get the broad patterns right (is it more right than wrong, or more wrong than right)? Do you think the model-data misfits might be important? How so?
...
\vspace{1mm}
\noindent You have been provided with an example \textit{user-config} -- \textsf{\footnotesize LAB.8.2} -- exactly the same as used to generate the (2nd stage) spin-up from \textit{Ridgwell and Hargreaves} [2007], except it also specifies the emission of $CO_{2}$ to the atmosphere -- \textsf{\footnotesize worbe2.FpCO2\_Fp13CO2\_PgC.detplusopalSED} -- exactly as done earlier, with the one exception that in addition to a generic (idealized) $CO_{2}$ forcing flux, a spatial pattern of detrial input to marine sediments is prescribed as part of the \textit{forcing}.

As before, the scaling parameter:
\vspace{-1mm}\small\begin{verbatim}
bg_par_atm_force_scale_val_3=0.0
\end{verbatim}\normalsize\vspace{-1mm}
sets the rate of carbon release, in $PgCyr^{-1}$, and the parameter:
\vspace{-1mm}\small\begin{verbatim}
bg_par_atm_force_scale_time_3=1.0
\end{verbatim}\normalsize\vspace{-1mm}
scales the duration over which the emissions occur.

The default settings in \textsf{\footnotesize LAB.8.2} are for $0PgC$ total over $1yr$, so you will want to: (a) copy-rename \textsf{\footnotesize LAB.8.2} and (b) set a higher rate and/or longer duration, and then (c) run for e.g., $100$ years and see if you can see anything happening:

\vspace{-1mm}\small\begin{verbatim}
./runcookie.sh cookie.CBSR.p_worbe2.BASES LABS LAB.8.2 100 
   cookie.CBSR.p_worbe2.BASES.RidgwellHargreaves.SPIN2
\end{verbatim}\normalsize\vspace{-1mm}
...
\textbf{Q.} Did you see much happens re. ocean-sediment interactions in $100$ years? If not, why?
\\Read \textit{Lord et al.} [2015] and \textit{Ridgwell and Hargreaves} [2007] for an analysis and discussion of global carbon cycle response time-scales.

\vspace{1mm}
Don't forget to run a control experiment -- identical to your emissions experiment, but with $CO_{2}$ flux scaling zero to zero\footnote{You need not change the time scaling because it does not matter how long zero emissions occur over :o)}:
\vspace{-1mm}\small\begin{verbatim}
bg_par_atm_force_scale_val_3=0.0
\end{verbatim}\normalsize\vspace{-1mm}
...
\noindent If you want a more geologically-relevant emissions scenario, e.g.:
...
\item Specifying:
\vspace{-1mm}\small\begin{verbatim}
bg_par_forcing_name='worbe2.FpCO2_Fp13CO2_PgC.detplusopalSED'
bg_par_atm_force_scale_val_3=0.1
bg_par_atm_force_scale_val_4=-15.0
\end{verbatim}\normalsize\vspace{-1mm}
creates a release of \(CO_{2}\) to the atmosphere at a rate of \(0.1 PgC yr^{-1}\) with an isotopic composition of \(-15\)\permille -- appropriate for a volcanic-related geological source. The line:
\vspace{-1mm}\small\begin{verbatim}
bg_par_atm_force_scale_time_3=10000.0
\end{verbatim}\normalsize\vspace{-1mm}
then specifies that this input (and isotopic composition) continues over 10,000 years.
...
\vspace{-1mm}\small\begin{verbatim}
bg_par_forcing_name='worbe2.FpCO2_Fp13CO2_PgC.detplusopalSED'
bg_par_atm_force_scale_val_3=50.0
...
\subsection{Extreme saturation and micrite deposition}

\textbf{cookie} can simulate the precipitation of $CaCO_{3}$ within the water column in the absence of an explicit biological driver (i.e., non-biogenic/abiotic carbonate). The parameter that enables water column $CaCO_{3}$ precipitation is:

\vspace{-2mm}\small\begin{verbatim}
# turn on abiotic CaCO3 precipitation
bg_ctrl_bio_CaCO3precip=.true.
\end{verbatim}\normalsize\vspace{-2mm}
Where precipitation occurs can then be restricted to just the ocean surface (rather than the entire water column) by:

\vspace{-2mm}\small\begin{verbatim}
# restrict precipitation to surface layer?
bg_ctrl_bio_CaCO3precip_sur=.true.
\end{verbatim}\normalsize\vspace{-2mm}
(the default setting).

$CaCO_{3}$ precipitation occurs in the model when the saturation state for calcite (or aragonite\footnote{Controlled by \texttt{sg\_par\_bio\_CaCO3precip\_calcite}}) exceeds some critical threshold:

\vspace{-2mm}\small\begin{verbatim}
# saturation threshold for abiotic CaCO3 precipitation
bg_par_bio_CaCO3precip_abioticohm_min=8.0
\end{verbatim}\normalsize\vspace{-2mm}
(here for $\Omega = 8.0$).

Finally, the scaling of precipitation rate, and how non-linear precipitation rate is with increasing over-saturation, are controlled by:

\vspace{-2mm}\small\begin{verbatim}
# CaCO3 precipitation kinetic controls
bg_par_bio_CaCO3precip_sf=2.0E-6
bg_par_bio_CaCO3precip_exp=2.0
\end{verbatim}\normalsize\vspace{-2mm}
...
\noindent Experiment \textsf{\footnotesize LAB.8.5.1} includes the parameter settings needed for water column precipitation of $CaCO_{3}$ to occur. Furthermore, \textsf{\footnotesize LAB.8.5.1} includes a salinity \textit{forcing} of the Eastern Mediterranean and explicitly links changes in the concentration of all dissolved species to the applied salinity change, so as to enable you to easily create the surface seawater conditions of carbonate super-saturation that are needed for saturation to exceed the critical threshold (determined by \texttt{\small bg\_par\_bio\_CaCO3precip\_abioticohm\_min=8.0}) and hence for abiotic precipitation to occur:

\vspace{-2mm}\small\begin{verbatim}
# apply salinity anomoly to the Eastern Mediterranean surface
bg_par_forcing_name="p_worxx2.Fsal_SUR_EMed"
# scale changes in all dissolved tracers along with salinity forcing
bg_ctrl_force_ocn_Sall=.true.
\end{verbatim}\normalsize\vspace{-2mm}

By default, the magnitude of the salinity \textit{forcing} is configured to be equivalent to $1 Sv$ (a very large salt/freshwater transfer) and because the selected \textit{forcing} represents a pulse of duration of $1 yr$, the duration in time is scaled by $\times 100$. The respective parameter settings for the salinity forcing are hence:

\vspace{-2mm}\small\begin{verbatim}
# scale the salinity forcing in units of Sv
bg_par_ocn_force_scale_val_2=1.0
# scale the duration of the forcing (years)
bg_par_ocn_force_scale_time_2=100.0
\end{verbatim}\normalsize\vspace{-2mm}
...
\noindent You can run this experiment from a \textit{restart}\footnote{\texttt{wget --no-check-certificate \\http://www.seao2.info/cgenie\_output/cookie.CB.p\_worjh2.CARB13C44Ca.SPIN.tar.gz} \\and then \\\texttt{tar xfzv cookie.CB.p\_worjh2.CARB13C44Ca.SPIN.tar.gz}} by:

\vspace{-1mm}\small\begin{verbatim}
./runcookie.sh cookie.CBSR.p_worjh2.CARB13C44Ca LABS LAB.8.5.1 200 
   cookie.CB.p_worjh2.CARB13C44Ca.SPIN
\end{verbatim}\normalsize\vspace{-1mm}

Here, the example of a $200$ year-long experiment is given, which in the context of the parameter values in \textsf{\footnotesize LAB.8.5.1} is sufficient to allow super-saturation to have built up over the first $100$ years and $CaCO_{3}$ precipitation to occur. During the second $100$ years, salinity (and carbonate saturation) starts to recover and precipitation stops.  Carbon and calcium isotopes are included only for illustration, with the $^{44}Ca$ fractionation being set to a wholly unrealistically large value just for the sake of something happening in such a geologically short experiment:

\vspace{-1mm}\small\begin{verbatim}
# 44Ca fractionation into CaCO3
# NOTE: default value is -1.09 (for forams, from Holmden et al. [2012])
bg_par_d44Ca_CaCO3_epsilon=-100.0
\end{verbatim}\normalsize\vspace{-1mm}
...
\noindent Run this and go find the sediment core output-- \textsf{\footnotesize fields\_sediments\_2d.nc} (refer to the beginning of this chapter) -- remembering that you can orientate yourself as to where the core locations are, by viewing variable \textsf{\footnotesize grid\_ID\_sedcore} in \textbf{netCDF} file \textsf{\footnotesize fields\_sediments\_2d.nc}.

\vspace{1mm}
\noindent Start with the change with depth (down-core) in the concentration (wt\%) of $CaCO_{3}$.
\vspace{1mm}
\\\textbf{Q.} Does $CaCO_{3}$ deposition occur at the same depth in the sediment column at all core locations? If not, why not? And are the maximum values in $CaCO_{3}$ the same in each core? (And if not, why not?)

Also view the $\delta ^{44}Ca$ variable.
\vspace{1mm}
\\\textbf{Q.} Why does $\delta ^{44}Ca$ change with depth in the sediments?

\vspace{1mm}
Note that you might want to change the scale of the sediment core values, and/or reduce the maximum y-axis length in \textbf{Panoply} to expand the stratigrahic column (navigate to the \textsf{\footnotesize Grid} plotting controls). You can also view the actually values -- in the plotting window, click on the tab \textsf{\footnotesize Array 1}.

\vspace{1mm}
As a 'fun' exercise -- try plotting bulk carbonate content and its isotopic composition as a function of age (e.g., by copying the age values of a column into \textbf{Excel} (/\textbf{MATLAB}/\textbf{python}) as the \textit{x}-axis column, and e.g., $CaCO_{3}$ wt\% as the 2nd, \textit{y}-axis column). For this, two different age models are provided:

\vspace{1mm}
\begin{enumerate}[noitemsep]
\item \textsf{\footnotesize age\_CaCO3}
\\The time at which the $CaCO_{3}$ is precipitated (and then deposited in accumulating sediments) and is in effect the 'true' mean age of $CaCO_{3}$ down-core.
\item \textsf{\footnotesize age\_ash}
\\Is a more practical age model and assume that the sediment accumulation rate is constant, taking the depth at which the experiment started from the stratigraphic location of the ash layer (variable: \textsf{\footnotesize sed\_ash}).
\end{enumerate}
\vspace{1mm}

You might try running the experiment for longer (perhaps a few thousand years), and/or extending the duration of the salinity forcing \textsf{\footnotesize bg\_par\_ocn\_force\_scale\_time\_2}.

%------------------------------------------------
\newpage
%------------------------------------------------

\noindent Lastly, you can adjust the \textit{user-config} (but first remember to copy-rename \textsf{\footnotesize LAB.8.5.1}) to use a salinity \textit{forcing} that consists of a series ($10$) of pulses (rather than just one). If you adopt the same pulse scaling (\texttt{\small bg\_par\_ocn\_force\_scale\_time\_2=100.0}) and run the experiment for $2000$ years, you can fit a total of $10$ salinity pulses in during the course of the experiment. Or increase the value of \texttt{\small bg\_par\_ocn\_force\_scale\_time\_2=100.0} and have fewer pulses of deposition occurring in the same $2000$ year-long experiment (remembering that you have already simulated a single depositional event).

To enable multiple pulses of deposition, simply change the parameter name of the \textit{forcing} at the start of the \texttt{\small FORCINGS} section of the \textit{user-config} as follows:

\vspace{-2mm}\small\begin{verbatim}
# apply salinity anomoly to the Eastern Mediterranean surface
bg_par_forcing_name="p_worxx2.Fsal_SUR_EMed_n10"
\end{verbatim}\normalsize\vspace{-2mm}

\vspace{1mm}
\noindent \textbf{Q.} Are all the pulses of equal magnitude (both in bulk and isotopic sediment composition)? If not, why not?
...
\subsection{Silicate weathering feedback sensitivity}
