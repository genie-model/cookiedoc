...
\chapter{Ocean circulation II}\label{ch:ocean-circulation-II}
...
You are going to need \textbf{MATLAB}, and then clone some (\textbf{MATLAB}) code ... but onto your local computer (\uline{not} the GENIE computing cluster). Or instead of cloning, downloading and unpacking the code archive will work just fine.
...
\begin{itemize}[noitemsep]
\vspace{1mm}
\item In the \textsf{\footnotesize results} output folder, there is a \textit{time-series} file named: \\\textsf{\footnotesize timeseries\_climate\_opsi.txt} \\This contains a summary of the evolution with time, of the minimum and maximum (anywhere) global overturning stream-function values.
\vspace{1mm}
\item If you have the ventilation age tracer included, there will also be a \textit{time-series} output:  \\\textsf{\footnotesize timeseries\_climate\_ventilation\_age.txt} with a field for: \\\texttt{\small benthic [> 2000 m] ventilation age (yr)}\footnote{If you add the parameter setting: \texttt{\small Adding ctrl\_save\_hidden\_extra=.true.} to your \textit{user-config} will result in additional data columns saved.} which is the mean age of water bathing the seafloor deeper than 2000 m (a depth cutoff which can be adjusted).
\vspace{1mm}
...
\\The presence/absence of sea-ice is also important and related to latitudinal temperature gradients and to the location and efficiency of deep-water formation (why?). 
\end{itemize}
...
For example -- the \textsf{\footnotesize drakeworld } configuration gives rise to a global MOC pattern as shown in Figure \ref{fig:paleo_drakeworld.opsi}. This is stored as the variable \textsf{\footnotesize climate\_opsi} in the netCDF file \textsf{\footnotesize fields\_ocean\_2d.nc}. Remember that the first time-slice plotted in \textbf{Panoply} is the first time-slice saved by default.
...
You can also plot the barotropic circulation using \textbf{Panoply} -- the variable is called \textsf{\footnotesize climate\_psi} -- this is shown in Figure \ref{fig:paleo_drakeworld.psi}.
...
A broader (or more fun) approach is to generalize the problem and consider fundamentally different configurations of ocean basins and climate. This is what you will be doing. But first, some potted advice:
...
\vspace{1mm}
\item Create a plan of study -- what sort of experiments, now many, how are they going to be analyzed, how do they all fit together in addressing the overarching hypotheses? Again -- it is very likely that your plan will evolve as you go along, but try and start out with something to guide you rather than wandering randomly in model world ...
\vspace{1mm}
...
\\As before, this (creating or changing the continental configuration used by \textbf{cookie}) is all done using the \textbf{MATLAB} \textbf{cookiegen} program. Any of the given example \textbf{cookiegen} configurations: \textsf{\footnotesize drakeworld}, \textsf{\footnotesize eqpasworld}, \textsf{\footnotesize ridgeworld}, or \textsf{\footnotesize waterworld}, could be taken as starting points -- copying and renaming the configuration \textsf{\footnotesize .m} file, as well as the \textsf{\footnotesize .dat} file in the \textsf{\footnotesize INPUT} directory.
...
\end{verbatim}\normalsize\vspace{-1mm}
in the configuration \textsf{\footnotesize .m} file.
\vspace{1mm}
