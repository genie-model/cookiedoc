...
\vspace{1mm}
Additional (netCDF) output has also been prescribed, via the \textit{user-config} parameter setting:

\vspace{-2mm}\small\begin{verbatim}
bg_ctrl_save_basic_reservoirs=.TRUE.
bg_ctrl_save_basic_geochemistry=.TRUE.
\end{verbatim}\normalsize\vspace{-2mm}
so that more data relevant to assessing carbonate chemistry and ocean acidification is saved.

%------------------------------------------------
\vspace{1mm}\noindent\rule{4cm}{0.1mm}\vspace{2mm}
%------------------------------------------------

\noindent First, you should start with a control experiment to provide a baseline response (or hopefully, lack of response) to which you can contrast your perturbation experiments. It is also a good experiment to interrogate and explore new output and concepts (i.e., carbonate chemistry). For this, copy and rename \textsf{\footnotesize LAB.5.1} to e.g., \textsf{\footnotesize LAB.5.1.control} and run for e.g., 50 years:

\vspace{-2mm}\small\begin{verbatim}
$ ./runcookie.sh cookie.CB.p_worjh2.rCARB LABS LAB.5.1.control 50 
   cookie.CB.p_worjh2.rCARB.SPIN
\end{verbatim}\normalsize\vspace{-2mm}

\noindent Now, you can run an actual experiment -- copy/rename \textsf{\footnotesize\ LAB.5.1} again (e.g., \textsf{\footnotesize\ LAB.5.1.emissions}) and find the parameter section under the heading \texttt{\small FORCINGS}:
...
In any of the emissions experiments you have tried out, in the time-series of atmospheric \(pCO_{2}\) (file: \textsf{\footnotesize timeseries\_atm\_pCO2.txt}) you'll undoubtedly see atmospheric \(pCO_{2}\) initially rise, but then once the emissions cease, start decaying back down again. Where is it 'going'?
...
Are there any other carbonate system impacts you can discern? Some 3D ocean fields to consider might include:
