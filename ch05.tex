...
In the next section will be chucking \(CO_{2}\) into the atmosphere, just for the hell of it. As much as you want! Apparently, humans are actually doing this now. Imagine that!

%------------------------------------------------

\subsection{Idealized emissions forcing (1)}

A new \textit{user-config} is provided -- \textsf{\footnotesize\ LAB.5.1.1}. Note that additional (time-series and netCDF) output has been prescribed, via the \textit{user-config} parameter settings:

\vspace{-2mm}\footnotesize\begin{verbatim}
bg_ctrl_save_basic_reservoirs=.TRUE.
bg_ctrl_save_basic_geochemistry=.TRUE.
\end{verbatim}\normalsize\vspace{-2mm}
so that more information relevant to assessing carbonate chemistry and ocean acidification is saved.

%------------------------------------------------
\vspace{1mm}\noindent\rule{4cm}{0.1mm}\vspace{2mm}
%------------------------------------------------

\noindent First, you should start with a control experiment to provide a baseline response (or hopefully, lack of response) to which you can contrast your perturbation experiments. It is also a good experiment to interrogate and explore new output and concepts (i.e., carbonate chemistry) with.

Copy and rename \textsf{\footnotesize LAB.5.1.1} to e.g., \textsf{\footnotesize LAB.5.1.1.control} and run for e.g., 50 years:

\vspace{-2mm}\small\begin{verbatim}
$ ./runcookie.sh cookie.CB.p_worjh2.rCARB LABS LAB.5.1.1.control 50 
   cookie.CB.p_worjh2.rCARB.SPIN
\end{verbatim}\normalsize\vspace{-2mm}

\noindent Next, you can run an actual experiment -- copy/rename \textsf{\footnotesize\ LAB.5.1.1} again (e.g., to \textsf{\footnotesize\ LAB.5.1.1.emissions}) and find the parameter section under the heading \texttt{\small FORCINGS}:

\vspace{-2mm}\footnotesize\begin{verbatim}
# SPEFICY ANY FORCINGS OF OCEAN/ATMOSPHERE/SEDIMENTS
# specify a flux of CO2 to atmosphere (an assign it an isotopic composition)
bg_par_forcing_name="pyyyyz.FpCO2_Fp13CO2_PgC"
# SCALE THE FORCING 
# NOTE: bg_par_atm_force_scale_val_3 is the flux in PgC
# NOTE: bg_par_atm_force_scale_time_3 is the duration of emissions in years
bg_par_atm_force_scale_val_3=0.0
bg_par_atm_force_scale_time_3=1.0
\end{verbatim}\normalsize\vspace{-2mm}

Here: \texttt{\small bg\_par\_atm\_force\_scale\_val\_3} is how many $PgC yr^{-1}$ of $CO_2$ you are going to release to the atmosphere, and \texttt{\small bg\_par\_atm\_force\_scale\_time\_3} is over how many years.\footnote{Technically, both parameters scale the values contained in the \textit{forcing}.} Pick a rate, and duration (no longer than the total experiment duration), and run your experiment. One strategy is to release \(CO_{2}\) just over a single year. While this is obviously rather unrealistic and many impacts will decay away rapidly afterwards, it represents a useful idealized experiment for assessing the time-scale(s) of fossil fuel \(CO_{2}\) uptake by the ocean. (For reference -- current emissions are ca. \(10\:PgC\:yr^{-1}\)) and have occurred on a time-scale of ca. 50 years.)

\vspace{1mm}
\noindent Run for the same duration as your control experiment (which should have been 50 years) and also starting from the same \textit{re-start} experiment \textsf{\footnotesize cookie.CB.p\_worjh2.rCARB.SPIN}, i.e.:

\vspace{-2mm}\small\begin{verbatim}
$ ./runcookie.sh cookie.CB.p_worjh2.rCARB LABS LAB.5.1.1.emissions 50 
   cookie.CB.p_worjh2.rCARB.SPIN
\end{verbatim}\normalsize\vspace{-2mm}
...
\subsubsection*{Where has my carbon gone???}

In your emissions experiment -- in the time-series of atmospheric \(pCO_{2}\) (file: \textsf{\footnotesize timeseries\_atm\_pCO2.txt}) -- depending on your carbon emissions duration, you'll will see atmospheric \(pCO_{2}\) initially rise, but then once the emissions cease, start decaying back down again. Where is it 'going'?
...
\noindent What about the other carbonate system parameters? Can you track identify patterns of uptake and ocean circulation transport. For instance \(CO_{2(aq)}\)? What about \(CO^{2-}_{3}\) (before looking, and remembering your basic carbonate chemistry, what would you expect)? (\(HCO^{2-}_{3}\) should primarily track \(DIC\).) Are there any other carbonate system impacts you can discern? Some 3D ocean fields to consider might include:

\vspace{1mm}
\begin{itemize}[noitemsep]
\item \(pH\) -- variable \textsf{\footnotesize carbchem\_pHsws} 
\\(There is also a field for the hydrogen ion concentration if you really want to see it.)
\item Calcite saturation state (\(\Omega_{(cal)}\)) -- variable \textsf{\footnotesize carbchem\_ohm\_cal}
\item Aragonite saturation state (\(\Omega_{(arg)}\)) -- variable \textsf{\footnotesize carbchem\_ohm\_arg} 
\end{itemize}
...
\noindent To track the fate of carbon -- also see the subsection -- "\textit{Isotopic tracing of fossil fuel CO$_{2}$ uptake}".
...
\vspace{1mm}
\noindent \uline{Before moving on}, ensure that you have developed a strategy for what you are 'looking for' in the model output.
...
Your experiment differs from those in previous Chapters because it is configured with climate being responsive to any changes in atmospheric \(CO_{2}\) (i.e., it takes account of \(CO_{2}\)-climate feedback). (We can make this choice now because we have a carbon cycle and can calculate atmospheric $CO_{2}$ in the first place.) The setting (near the top of the \textit{user-config} file) that does this is:
\vspace{-2mm}\small\begin{verbatim}
...
\vspace{1mm}
The presence of feedback between climate and carbon emissions complicates understanding of the response. For instance, additional carbon will enter the atmosphere from the ocean due to surface warming and hence the value of atmospheric \(CO_{2}\) at any one time may be higher than you would get from emissions alone. A great advantage of numerical models is the ability to pick apart feedbacks and isolate different mechanisms and dynamics. In this example, we can remove the climate feedback and analyze the response of the global carbon cycle to emissions in the absence of a changing climate.

\vspace{1mm}
To do this, in a new \textit{user-config} file, simply set:
\vspace{-2mm}\small\begin{verbatim}
ea_36=n
\end{verbatim}\normalsize\vspace{-2mm}
and then re-run your emissions experiment.

\vspace{1mm}
\noindent \textbf{Q. How and why do the experiments differ?}

%------------------------------------------------
\newpage
%------------------------------------------------

\subsection{Idealized emissions forcing (2)}
...
\subsection{Historical (real-world!) greenhouse gas changes}

Historical and future (e.g. IPCC 'SRES') emissions scenarios can  be prescribed explicitly and simply in \textbf{cookie}. An example is given as \textit{user-config} file: \textsf{\footnotesize LAB.5.1.3}. In this, a historical emissions forcing (technically: a prescribed concentration profile of \(pCO_{2}\) is specified by the \textit{forcing}:
\vspace{-2pt}\small\begin{verbatim}
...
\end{verbatim}\normalsize\vspace{-2pt}
specifies a series of time points at which data is saved that aligns with historically relevant years.\footnote{  These files: \textsf{save\_timeslice\_historicalfuture.dat} and \textsf{save\_timeseries\_historicalfuture.dat}live in the directory: \textsf{genie-biogem/data/input}, and can be edited to add or remove different years to save data at, or copied and renamed, with the new file-names substituted in the two \textit{user-config} parameter settings.}

Copy and rename \textsf{\footnotesize LAB.5.1.3} to something more meaningful, e.g., \textsf{\footnotesize LAB.5.1.3.historical}.
...
\vspace{-2pt}\small\begin{verbatim}
$ ./runcookie.sh cookie.CB.p_worjh2.rCARB LABS LAB.5.1.3.historical 245
   cookie.CB.p_worjh2.rCARB.SPIN
\end{verbatim}\normalsize\vspace{-2pt}
The \texttt{245} parameter value for experiment duration (years) arises on the basis of the start year being \(1765\) (as specified by \texttt{\small bg\_par\_misc\_t\_start=1765.0}) and to give an experiment end at year \(2010\).
...
\noindent There is observationally-based information on the distribution of anthropogenic \(CO_{2}\) taken up by the ocean (e.g., \textit{Sabine et al.} [2004]) ... and you are running a historical transient experiment with the model driven by observed increases in atmospheric p\(CO_{2}\) ... so you are in a position to critically evaluate the models ability (or lack of) to represent the future-critical process of oceanic fossil fuel \(CO_{2}\) uptake and transport by large scale ocean circulation.

%------------------------------------------------
\newpage
%------------------------------------------------

Observational data, re-gridded to the \textbf{cookie} grid and in netCDF format can be downloaded from: \href{http://www.seao2.org/mymuffin.html}{http://www.seao2.org/mymuffin.html} (and under the ‘got data?’ box on the left). You could for instance, compare horizontal or vertical slices (3D netCDF) and create difference (anomaly) maps. Because the data in the Sabine paper clusters around 1994, a \textit{time-slice} is deliberately saved centered on this year (1994.5). Your baseline state can either be from prior to \(CO_{2}\) emissions commencing at any significant rate (e.g., 1750.5) or (better), from a control experiment. Note that similar comparisons could be (and are regularly) made with other tracers such as CFCs, which provide additional insights into the patterns and time-scales of trace gas update and ocean circulation. (See: \textit{Cao et al.} [2009])

\vspace{2mm}

\begin{figure}[ht]
\begin{center}
\includegraphics[scale=0.6]{chx-co2uptake.pdf}
\end{center}
\vspace{-10pt}
...
\noindent Note that model and data are not strictly comparable in the particular configuration used here, as we do not have any biology or a 'biological pump' (more on this later) in the ocean. (The example in the figure uses the with-biology biogeochemical configuration described by \textit{Cao et al.} [2009].) Also note that the model and observations are processed here as water-column integrals using \textbf{MATLAB}.
...
These are different plausible 'future' emissions scenarios, derived from different socio-economic and future technological assumptions in making their future emissions projections. They all start at year 2010 and end at year 2100.

\vspace{1mm}
These particular \textit{forcings} use units of \(PgCyr^{-1}\) in the \textit{forcing} \textit{time-series} files, so you will need to ensure that the scaling parameter in your \textit{user-config} file is set to turn units of \(PgCyr^{-1}\) into \(mol C yr^{-1}\):\footnote{For completeness also add in the specification of the isotopic composition of the carbon emissions -- refer back to the idealized experiments.}
\vspace{-1mm}\small\begin{verbatim}
bg_par_atm_force_scale_val_3=8.3333e+013
\end{verbatim}\normalsize\vspace{-1mm}

You will want to run your experiment starting from the end of the historical transient experiment you have just run rather than the original steady-state \textit{re-start}:\footnote{Note that the \textit{user-config} \textsf{\footnotesize LAB.5.1.4.future } is not provided for you – you will need to create this (or a file named whatever you like) by copying e.g., \textsf{\footnotesize LAB.5.1.4.future} and making the parameter changes described above (forcing specification parameter, emissions scaling parameter, and start year parameter).}
\vspace{-1mm}\small\begin{verbatim}
$ ./runcookie.sh cookie.CB.p_worjh2.rCARB LABS LAB.5.1.4.future 90 LAB.5.1.3.historical
\end{verbatim}\normalsize\vspace{-1mm}
and to run for 90 years from 2010 to 2100, set the start year to the year that the previous historical transient finished on (\(2010\)):
\vspace{-1mm}\small\begin{verbatim}
...
the scaling parameters, if set, are either commented out, e.g.:
\vspace{-2pt}\small\begin{verbatim}
#bg_par_atm_force_scale_val_3=8.3333e+013
...
\vspace{1mm}
Note that the equivalent carbon emissions required to follow these concentration pathways are diagnosed by \textbf{cookie} and provided as a \textit{time-series} output (in units of \(mol C yr^{-1}\)) in file: 
\vspace{1mm}
...
\noindent The  experiments on fossil fuel \(CO_{2}\) emissions to the atmosphere include an assumed isotopic composition of the emitted carbon. For any of the carbon emissions experiments (including the historical transient) it is possible to trace how the isotopic composition of fossil fuel carbon is propagated from the atmosphere into the ocean and hence throughout the ocean via its large-scale circulation. However, by default, it is not saved in the output, so you will need to change one of the parameter value settings in your \textit{user-configs}, and then re-run all the experiments that you want to analyze \(\delta^{13}C\) in:

\vspace{-1mm}\small\begin{verbatim}
bg_ctrl_save_basic_proxies=.true.
\end{verbatim}\normalsize\vspace{-1mm}

The key variable to plot (but also see the Appendix at the end of this Chapter) is: \textsf{\footnotesize ocn\_DIC\_13C}\footnote{Also calculated and saved are the isotopic compositions of the 3 different aqueous carbonate chemistry components -- \textsf{\footnotesize carb\_d13C\_CO2}, \textsf{\footnotesize carb\_d13C\_CO32}, and \textsf{\footnotesize carb\_d13C\_HCO3}.}. In this, it is helpful to also take a control experiment (you did run one, right ... ?) and create a difference map to better visualize how the \(\delta^{13}C\) patterns in the ocean evolve through time.

%------------------------------------------------
\vspace{1mm} \noindent\rule{4cm}{0.1mm} \vspace{2mm}
%------------------------------------------------

\noindent By default, fossil fuel carbon is tagged with a mean fossil fuel isotopic signature of \(-27\)\permille. This is set by the parameter:
\vspace{-1mm}\small\begin{verbatim}
bg_par_atm_force_scale_val_4=-27.0
\end{verbatim}\normalsize\vspace{-1mm}
which scales the value of \(1.0\) specified in the file \textsf{\footnotesize biogem\_force\_flux\_atm\_pCO2\_13C\_sig.dat} in the \textit{forcing} directory, which in the previous fossil fuel release experiments was generally the \textsf{\footnotesize genie-forcing} sub-directory:  \textsf{\footnotesize pyyyyz.FpCO2\_Fp13CO2\_PgC} or \textsf{\footnotesize pyyyyz.FpCO2\_Fp13CO2}.

\vspace{1mm}

To create a more pronounced 'tag' (or tracer) of fossil fuel carbon, you could, for instance, make the assumed value of the \(CO_{2}\) more negative, e.g. \(-60\)\permille would be the signature of methane (natural gas). You could also push the value even more negative and consider it an idealized numerical tracer of fossil fuel carbon (note that the lowest value you are allowed to set is \(-999\)\permille) -- in this case, you need only emit very little carbon (<< $1PgC$) and hence hardly perturb the steady-state aqueous carbonate system or climate, and still be able to trace the update of fossil fuel carbon.
...
\begin{table}[ht!]
\begin{tabular}{p{0.275\linewidth} p{0.30\linewidth} p{0.35\linewidth}}
\toprule
\textbf{Filename} & \textbf{Description} & \textbf{Application}\\
\textsf{\footnotesize timeseries\_*.txt} & &\\
\midrule
...
\textsf{\footnotesize carbchem\_seasur\_conc\_*} & \small{Carbonate chemistry components (mean surface) (\(molkg^{-1}\)).} & \small{Not generally useful. }\\
\textsf{\footnotesize carbchem\_seasur\_pHsws} & \small{Surface ocean mean pH} & Ocean acidification.\\
\textsf{\footnotesize carbchem\_seasur\_ohm\_arg} & \small{Mean surface aragonite saturation.} & \small{Ocean acidification impacts of \(CO_{2}\) release. Weathering impacts. Relates to carbonate production by (modern) corals, pteropods.}\\
\textsf{\footnotesize carbchem\_seasur\_ohm\_cal} & \small{Mean surface calcite saturation.} & \small{Ocean acidification impacts of \(CO_{2}\) release. Weathering impacts. Carbonate production by foraminifera and coccolithophorids.}\\
...
\bottomrule
\end{tabular}
\caption{Summary of the main/useful \textit{time-series} output for (bio)geochemistry (non ecological) investigations.}
\end{table}
...
\begin{table}[ht!]
\begin{tabular}{p{0.30\linewidth} p{0.30\linewidth} p{0.325\linewidth}}
\toprule
\textbf{Variable} & \textbf{Description} & \textbf{Application}\\
\midrule

\textsf{\footnotesize carbchem\_seasur\_ohm\_arg} \textsf{\footnotesize carbchem\_seasur\_ohm\_cal} & \small{Ocean surface aragonite and calcite saturation.} & \small{Impacts of ocean acidification of distribution of planktic organisms.}\\

\midrule

\textsf{\footnotesize CO2\_Dairsea} & \small{Air-sea \(CO_{2}\) difference.} & \small{Indicator of air-sea gas disequilibrium, regions of out-gassing/in-gassing.}\\

\midrule

\textsf{\footnotesize carbchem\_seasur\_pHsws} & \small{Ocean surface pH.} & \small{Ocean acidification.}\\

\midrule

\textsf{\footnotesize CO2\_wcint\_DIC} & \small{Pattern of water column integrated ocean \(DIC\) (i.e. dissolved carbon storage) (\(molm^{-2}\)).} & \small{Indicator of \(CO_{2}\) emissions storage and transport when used in difference/anomaly maps and calculations.}\\
...
\textsf{\footnotesize ocn\_DIC} & \small{Distribution of ocean \(DIC\) concentrations (\(molkg^{-1}\)).} & \small{Indicator of ocean carbon transport and the 'solubility pump'.}\\

\textsf{\footnotesize ocn\_DIC\_13C} & \small{Distribution of the \(\delta^{13}C\) of \(DIC\).} & \small{Indicator of ocean carbon transport and the 'solubility pump'.}\\

\bottomrule
\end{tabular}
\caption{Summary of the main (mostly useful) 3D time-slice output for (bio)geochemistry.}
\end{table}
