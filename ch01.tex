...
$ wget --no-check-certificate 
   http://www.seao2.info/cgenie_output/cookie.C.p_woreq1.r.SPIN.tar.gz
\end{verbatim}\normalsize\vspace{-2mm}
(all one line, space before \texttt{\small http})

\vspace{1mm}
This downloads an archived/compressed copy of the experiment \textsf{\footnotesize cookie.C.p\_woreq1.r.SPIN} – effectively, just an experiment (spin-up) that has already been run for 10,000 years for you. Extract the contents of this archive by typing (also from the \textsf{\footnotesize cgenie\_output} directory):

\vspace{-2mm}\small\begin{verbatim}
$ tar xfzv cookie.C.p_woreq1.r.SPIN.tar.gz
\end{verbatim}\normalsize\vspace{-2mm}

A new experiment results directory will then appear in \textsf{\footnotesize cgenie\_output} as if you had just run the entire 10,000 year experiment yourself! \uline{Remember} -- to refresh the \textbf{WinSCP} directory view window if you are using this particular software, or it might appear that nothing has been extracted.
...
\noindent \textbf{TLDR} -- \textsf{\footnotesize wget} to the directory \textsf{\footnotesize cgenie\_output} and then extract in that same directory. Change back to \textsf{\footnotesize genie-main} to run an experiment using this \textit{restart}.
...
To illustrate how ‘easy’ it can be to configure experiments in an Earth system / climate model such as \textbf{cookie} and explore the behavior of the Earth system and its response to perturbation – you are going to induce an extreme cooling of the climate system and see what happens. 
...
\noindent You are going to be running experiments in a similar manner to before, and using the \textit{re-start} experiment that you downloaded. You could then, for example, take the experiment \textit{user-config} file \textsf{\footnotesize LAB.1.1} (which is provided for you), and run the experiment, which has a new \textit{base-config}: \textsf{\footnotesize cookie.C.p\_woreq1.r}, by typing:
\vspace{-1mm}\small\begin{verbatim}
$ ./runcookie.sh cookie.C.p_woreq1.r LABS LAB.1.1 100 cookie.C.p_woreq1.r.SPIN
\end{verbatim}\normalsize
...
\subsection{Finding the threshold for entering a snowball}
...
which is set at the top of the provided \textit{user-config} file \textsf{\footnotesize LAB.1.1}. (For reference, the modern value is \( 1368 Wm^{-2}\).)
...
you are going to edit the experiment file that controls the specific details of the experiment -- the \textit{user-config} file. From the \textsf{\footnotesize genie-userconfigs/LABS} directory, open one of the snowball experiments in your preferred  text editor. Near the top of the file you should see:

\vspace{-2mm}\footnotesize\begin{verbatim}

# *** CLIMATE **********************************************************
#
# SPECIFY SOLAR CONSTANT (Wm-2)
# NOTE: default value is 1368.0
# e.g., ma_genie_solar_constant=1285.92 -> solar constant reduced by 5.6% for late Neoproterozoic
ma_genie_solar_constant=1285.92
...
# e.g., ea_radfor_scl_co2=20.0 -> radiative forcing equivalent to changing from x1 to x20 CO2
...
\noindent Each line that is not commented out (i.e., no \#) contains a \textit{parameter} name and assigned value pair, with the format:
\vspace{-1mm}\small\begin{verbatim}
PARAMETER=VALUE
\end{verbatim}\normalsize\vspace{-1mm}
...
\vspace{1mm}
\noindent Other questions to think about with regards to numerical modeling are:
...
\subsection{Finding the threshold for exiting a snowball}
...
\vspace{1mm}
\noindent HINT: If you are having trouble deciding whether or not the snowball is heading in the right direction (i.e. towards an exit!), e.g. because sea-ice is always reported at 100\% (or close to), you can keep track of whether there is net melting or net freezing by following mean sea-ice thickness (m). Your indication of a melting snowball state is a progressive decline in mean thickness (a proxy for global ice volume). \uline{Note that you can open and review the results of \textit{time-series} files at *any* time during the experiment as the lines are written while the data for each time point is generated.} However, by default, only the \% ice cover is reported in \textsf{\footnotesize timeseries\_climate\_seaice.txt} time-series output file. To get additional information saved, in your \textit{user-config} file, you need to add the line:

\vspace{-1mm}\small\begin{verbatim}
bg_ctrl_save_hidden_extra=.true.
\end{verbatim}\normalsize\vspace{-1mm}
...
\section{Further snowball Earth ideas}
...
It was mentioned earlier that the position of the continents is an area of modeling uncertainty and might be important. You can test for this. Four alternative \textit{base-configs} are provided, each defining a different continental configuration (each with a horizontal ocean resolution of 36x36, with 8 vertical levels):
...
You can use the given \textit{user-config} file (\textsf{\footnotesize LAB.1.1}) again as an experiment template file, and any of the alternative configurations can be run very similarly to as per before, i.e.:

\vspace{-2mm}\begin{verbatim}
$ ./runcookie.sh cookie.C.p_xxxxxx.r LABS LAB.1.1 100
...
\vspace{1mm}
\uline{Also note} that \uline{no} \textit{re-starts} are provided for any of the additional configurations (so excluding \textsf{\footnotesize p\_woreq1}). You may (or may not) want to create some (you will need to judge for yourselves how long to run the \textit{re-start} experiments to achieve as close to steady-state as you think is ‘sufficient’). Recall again, that \textit{re-starts} are just ‘normal’ experiments that have already been run.
...
\noindent The scientific question here in trying this would be whether or not taking into account a seasonally-varying climate substantially affects the entry (and/or exit) thresholds for a snowball climate state.
...
For this, the\textit{base-config} is named: \textsf{\footnotesize cookie.C.fm0635cb.r}

\vspace{1mm}
The \textit{user-config} is: \textsf{\footnotesize cookie.C.fm0635cb.r.SPIN}

\vspace{1mm}
A \textit{re-start }experiment is provided called \textsf{\footnotesize cookie.CB.fm0635cb.r.SPIN} and which can be downloaded as per before:

\vspace{-2mm}\small\begin{verbatim}
$ wget --no-check-certificate 
  http://www.seao2.info/cgenie_output/cookie.C.fm0635cb.r.SPIN.tar.gz
\end{verbatim}\normalsize\vspace{-2mm}
and unpack by:

\vspace{-2mm}\small\begin{verbatim}
$ tar xfzv cookie.C.fm0635cb.r.SPIN.tar.gz
\end{verbatim}\normalsize\vspace{-2mm}
(Remember: you should be in the \textsf{\footnotesize cgenie\_output} directory when you do this downloading and unpacking.)

To run (e.g. for 100 years), following on from its \textit{re-start}, and copying (/renaming) the same \textit{user-config} as before:

\vspace{-2mm}\small\begin{verbatim}
$ ./runcookie.sh cookie.C.fm0635cb.r LABS LAB.1.1 100
   cookie.C.fm0635cb.r.SPIN
\end{verbatim}\normalsize\vspace{-2mm}
...
\section{Further ideas -- Climate sensitivity}

This is a mini project to investigate climate sensitivity and the role of sea-ice feedback on climate -- specifically: whether climate sensitivity it is what is known as 'state-dependent'. 

%------------------------------------------------
\vspace{1mm}\noindent\rule{4cm}{0.5pt}\vspace{2mm}
%------------------------------------------------

\noindent You are going to need a new \textit{re-start}. So, from \textsf{\footnotesize cgenie\_output}:

\vspace{-2mm}\small\begin{verbatim}
$ wget --no-check-certificate 
http://www.seao2.info/cgenie_output/cookie.C.p_worjh2.r.SPIN.tar.gz
\end{verbatim}\normalsize\vspace{-2mm}
...
\noindent Take the \textit{user-config} \textsf{\footnotesize LAB.1.3}, and copy and rename it. Open the file in a tex editor, and in the section at the top:
...
# NOTE: the value of ea_radfor_scl_co2 represents a multiplier of CO2 relative to 278 ppm 
#       and radiative forcing is derived from this
# e.g., ea_radfor_scl_co2=1.0 -> radiative forcing equivalent to changing 
#       from x1 to x1 preindustrial CO2 (no change!)
ea_radfor_scl_co2=1.0
\end{verbatim}\normalsize\vspace{-1mm}

\noindent you are going to edit the parameter that -- in the absence of calculated $CO_2$ in the atmosphere (which requires a full carbon cycle) -- the radiative forcing that would result from an assumed change in atmospheric $CO_2$. The parameter \texttt{ea\_radfor\_scl\_co2} sets the assumed $CO_2$ value relative to pre-industrial $CO_2$ (278 ppm). So setting: \texttt{\small ea\_radfor\_scl\_co2=1.0} is the same as no change in radiative forcing (relative to pre-industrial) or $0 Wm^{-2}$. Whereas, a value greater than $1.0$ is interpreted as $CO_2$ being higher than pre-industrial and hence greenhouse warming, while lower than $1.0$ is $CO_2$ lower than pre-industrial and global cooling.

\vspace{1mm}
What value should you set \texttt{\small ea\_radfor\_scl\_co2} to? Please pick something between $0.5$ and $16.0$.

%------------------------------------------------
\vspace{1mm}\noindent\rule{4cm}{0.5pt}\vspace{2mm}
%------------------------------------------------

\noindent Run your experiment for 100 years starting from the \textit{restart}, like this:

\vspace{-2mm}\small\begin{verbatim}
$ ./runcookie.sh cookie.C.p_worjh2.r LABS LAB.1.3 100 cookie.C.p_worjh2.r.SPIN
\end{verbatim}\normalsize\vspace{-2mm}

\noindent (although you should have re-named your experiment something different from \textsf{\footnotesize LAB.1.2}.)
...
\noindent When the experiment has finished -- from the relevant \textit{time-series} files, note down the following values for the last saved year (99.5) of data:

\vspace{1mm}
\begin{itemize}[noitemsep]
\setlength{\itemindent}{.2in}
\item Global mean SAT.
\item Global mean SST.
\item Global mean percent (fractional) sea-ice cover.
\end{itemize}

\vspace{1mm}
And then ... create a new \textit{user-config}, and pick a different value of \texttt{\small ea\_radfor\_scl\_co2}.

\vspace{1mm}
And repeat. (For as long as you can be bothered.) Plot the 3 variables vs. $log2$ of the value of \texttt{\small ea\_radfor\_scl\_co2}.

%------------------------------------------------
\vspace{1mm}\noindent\rule{4cm}{0.5pt}\vspace{2mm}
%------------------------------------------------

\noindent \textbf{Q.} What is the shape of the relationship you observed between climate and ? Why?
