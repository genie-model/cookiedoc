...
\vspace{2mm}
\item [6.2] A single (\(PO_{4}\)) nutrient biological export scheme but in a lower (vertical) resolution ocean.
...
\end{verbatim}\normalsize\vspace{-1mm}
\vspace{2mm}
\item [6.3] These are paired T-dependent and non-T-dependent export and remineralizaton configurations.
\vspace{-1mm}\footnotesize \begin{verbatim}
$ wget --no-check-certificate 
...
\end{verbatim} \normalsize
\vspace{2mm}
\item [6.4] This is a scheme with T-dependent biological export (but with a non T-dependent, fixed remineralization profile) but with Fe co-limitation alongside that of P.
\vspace{-1mm}\footnotesize\begin{verbatim}
$ wget --no-check-certificate 
...
\section{The ocean's biological pump in cookie}

%------------------------------------------------

To start your tour of the oceans biological pump in cookie -- run a short (e.g., 10 years) experiment to generate some representative output. Copy-rename \textsf{\footnotesize LAB.6.1} to \textsf{\footnotesize LAB.6.1.control}, and:

\vspace{-2mm}\small\begin{verbatim}
$ ./runcookie.sh cookie.CB.p_worjh2.BASES LABS LAB.6.1.control 10 
   cookie.CB.p_worjh2.BASES.caoetal.SPIN
\end{verbatim}\normalsize\vspace{-2mm}

%------------------------------------------------

\subsection{cookie biological pump output}

%------------------------------------------------

\subsubsection*{What to look for?}
 
What should you be thinking of looking at in terms of model output to understanding something about the role of the oceans biological pump in the Earth system (and climate dynamics)?

\begin{itemize}[noitemsep]

\vspace{2mm}
\item \textbf{Summary diagnostics}
\vspace{1mm}
\\Firstly, there is summary text file saved at the same time as each \textbf{netCDF} \textit{time-slice}: 
\\\textsf{\footnotesize INFO\_ocean\_summary\_AT\_yyyy\_yyy\_yrs.txt}
where \textsf{\footnotesize yyyy\_yyy} is the mid-point of the annual average year saved.\footnote{In the example of your 50-year-long experiment, the file-name will be: \\\textsf{INFO\_ocean\_summary\_AT\_09\_500\_yrs.txt}.}  

\vspace{1mm}
Key metrics of the state of the oceans biological pump include:
...
\vspace{1mm}
Finally, there is a summary of the summary, which is perhaps the most useful of all:
\vspace{1mm}
\begin{itemize}[noitemsep]
\item
\vspace{-0mm}\footnotesize\begin{verbatim}
...
%
\vspace{2mm}
\item \textbf{\textit{Time-series}}
\vspace{1mm}
\\Secondly, there are a number of biological pump related \textit{time-series} outputs, e.g.: 
\vspace{1mm}
\begin{itemize}[noitemsep]
\item []
\begin{itemize}[noitemsep]
\item [] \textsf{\footnotesize timeseries\_biop\_fexport\_POC.txt}
\item [] \textsf{\footnotesize timeseries\_biop\_fexport\_POP.txt}
\item [] \textsf{\footnotesize timeseries\_biop\_fexport\_CaCO3.txt}
\end{itemize}
\vspace{1mm}
are the \textit{time-series} of the carbon and phosphorous of particulate organic matter export, and that for \(CaCO_{3}\), and
\vspace{1mm}
\begin{itemize}[noitemsep]
\item [] \textsf{\footnotesize biogem\_series\_ocnsed\_POC\_13C.res}
\item [] \textsf{\footnotesize biogem\_series\_ocnsed\_CaCO3\_13C.res}
\end{itemize}
\vspace{1mm}
are the corresponding carbon isotopic compositions (for: \texttt{\small bg\_ctrl\_save\_basic\_proxies=.true.}).
\\The \textit{time-series} output:
\begin{itemize}[noitemsep]
...
\vspace{2mm}
\item \textbf{netCDF} \textit{time-slices}
\vspace{1mm}
\\Thirdly, as always -- \textbf{netCDF} outputs.
\begin{itemize}[noitemsep]
\vspace{1mm}
\item In the 2D \textbf{netCDF}:
\vspace{1mm}
\begin{itemize}[noitemsep]
\item [] \textsf{\footnotesize biop\_seasur\_f\_POC}
\item [] \textsf{\footnotesize biop\_seasur\_f\_POP}
\item [] \textsf{\footnotesize biop\_seasur\_f\_CaCO33}
\end{itemize}
are spatial fields of export flux of \(POC\), \(POP\), \(CaCO_{3}\).

\vspace{1mm}
Derived from these:
\vspace{1mm}
\begin{itemize}[noitemsep]
\item [] \textsf{\footnotesize biop\_seasur\_r\_CaCO3toPOC}
\item [] \textsf{\footnotesize biop\_seasur\_r\_CtoP}
\end{itemize}
are the rations of \(CaCO_{3}/POC\) and \(POC/POP\), respectively.
\vspace{1mm}
\item In the 3D \textbf{netCDF}, \textsf{\footnotesize biop\_fsinking\_*} are the 3D spatial distributions of particle flux (density) through the water column.
\vspace{1mm}
...
\section{Exploring the controls on biological productivity}
...
%------------------------------------------------
%
\noindent In terms of the specific 'questions' at the start (the numbering system is the same in the following), the parameters and parameter values to change and well as what to 'look for' are as follows:
\vspace{1mm}
