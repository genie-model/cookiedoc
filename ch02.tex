...
The \textit{user-config} file for this is called: \textsf{\footnotesize LAB.2.5.hosing}. This includes a prescribed transport of salt to the North Atlantic (north of ca. 30$^{\circ}$N) from the result of the global surface ocean (for mass balance). This is specified by the line:
...
\noindent 10 years should be long enough to see a collapse start to occur, but you might want to run the model for longer (and it can be submitted as a job, of course). Running for longer will also allow you to have a smaller, less extreme (and maybe more realistic) freshwater input flux.
